    ...
    <div className="w-full space-y-6">
      <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-2xl backdrop-blur">
        <div className="mb-6 flex flex-col gap-2">
          <h2 className="text-2xl font-semibold text-white">Video Studio</h2>
          ...
          <div className="space-y-5">
            <div>
              ...
              <textarea
                ...
                rows={7}
                className="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-sm text-white outline-none transition focus:border-sky-500"
              />
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="flex items-end">
                <label className="flex w-full items-center gap-3 rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-sm text-slate-200">
                  <input
                    type="checkbox"
                    checked={withAudio}
            ...
            {mode === "image_to_video" && (
              <div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-4">
                ...
                <input
                  ...
                  className="block w-full text-sm text-slate-300 file:mr-4 file:rounded-xl file:border-0 file:bg-sky-600 file:px-4 file:py-2 file:text-white hover:file:bg-sky-500"
                />
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                disabled={!canSubmit}
                className="rounded-2xl bg-sky-600 px-5 py-3 text-sm font-medium text-white transition hover:bg-sky-500 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                ...
                className="rounded-2xl border border-slate-600 px-5 py-3 text-sm font-medium text-slate-200 transition hover:bg-slate-800"
              >
                Limpiar
              </button>
            </div>
          ...
          <div className="space-y-5">
            <div className="rounded-2xl border border-slate-700 bg-slate-950/70 p-4">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-white">Estado del job</h3>
              ...
              <div className="space-y-3 text-sm text-slate-300">
                <div className="flex items-center justify-between">
                  <span>Job ID</span>
                  <span className="max-w-[210px] truncate text-right text-slate-100">
                    {jobId ?? "—"}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="inline-flex items-center gap-2">
                    ...
                    <span className="text-slate-100">{statusText(jobStatus)}</span>
                  </span>
                </div>

                <div className="flex items-center justify-between">
                  <span>Polling</span>
                  <span className="text-slate-100">{isPolling ? "activo" : "inactivo"}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span>Modelo</span>
                  <span className="text-slate-100">{model ?? "—"}</span>
                </div>
            ...
            <div className="rounded-2xl border border-slate-700 bg-slate-950/70 p-4">
              <h3 className="mb-4 text-sm font-semibold text-white">Resultado</h3>
            ...
            <div className="rounded-2xl border border-slate-700 bg-slate-950/70 p-4">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-white">Jobs recientes</h3>
                <span className="text-xs text-slate-400">{recentJobs.length} items</span>
              </div>

              {recentJobs.length === 0 ? (
                <p className="text-sm text-slate-400">Todavía no hay jobs en esta sesión.</p>
              ) : (
                  ...
                  {recentJobs.map((job) => (
                    ...
                    >
                      <div className="mb-2 flex items-start justify-between gap-3">
                        <p className="line-clamp-2 text-sm text-slate-100">{job.prompt}</p>
                        <span className="inline-flex items-center gap-2 whitespace-nowrap text-xs text-slate-300">
                          <span className={`h-2.5 w-2.5 rounded-full ${statusColor(job.status)}`} />
                          {statusText(job.status)}
                        </span>
                      </div>

                      <div className="mb-3 text-xs text-slate-400">{job.id}</div>

                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => void handleReuseJob(job.id)}
                          className="rounded-xl border border-slate-600 px-3 py-2 text-xs text-slate-200 hover:bg-slate-800"
                        >
                          Ver estado
                        </button>
