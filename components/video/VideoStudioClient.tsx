...
export default function VideoStudioClient() {
  ...
  const canSubmit = useMemo(() => {
    ...
    if (mode === "image-to-video" && !uploadedImageUrl && !selectedFile) {
      return false
    }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const body = {
        ...
        mode: mode === "image-to-video" ? "image_to_video" : "text_to_video",
        imageUrl,
      ...
      if (data.videoUrl) {
        setCurrentJobId("instant-result")
        setCurrentStatus("completed")
        setCurrentOutputUrl(data.videoUrl)
      ...
      if (!data.jobId) {
        setCurrentError("La respuesta no incluyó jobId ni videoUrl.")
        return
      }
    ...
    setRecentJobs((prev) => [queuedJob, ...prev].slice(0, 8))
      ...
      console.error(error)
      setCurrentError("Ocurrió un error al enviar el video.")
    } finally {
      setIsSubmitting(false)
    }
  }
