...
const STORAGE_KEY = "eduai_litert_benchmark_v3"
const WARMUP_RUNS = 3
...
function summarizeRuns(backend: BackendName, compileMs: number, warmups: number[], runs: number[]): BackendResult {
  const averageMs = average(runs)
  const stdDevMs = standardDeviation(runs)
  return {
    backend,
    supported: true,
    compileMs,
...
function emptyResult(backend: BackendName, error: string): BackendResult {
  return { backend, supported: false, compileMs: 0, warmups: [], runs: [], averageMs: 0, medianMs: 0, p95Ms: 0, minMs: 0, maxMs: 0, stdDevMs: 0, cvPercent: 0, error }
}
...
export default function LiteRTBenchmarkPanelV3() {
  ...
  useEffect(() => {
    try {
      ...
      // El historial local no debe bloquear el laboratorio.
    }
  }, [])

  const graphMax = useMemo(() => latest ? Math.max(...latest.webgpu.runs, ...latest.wasm.runs, 1) : 0, [latest])
  ...
  async function runBackend(litert: any, backend: BackendName): Promise<BackendResult> {
    ...
    try {
      ...
      for (let index = 0; index < WARMUP_RUNS; index += 1) {
        setPhase(`${backend.toUpperCase()} · warm-up ${index + 1}/${WARMUP_RUNS}`)
      ...
      for (let index = 0; index < MEASURED_RUNS; index += 1) {
        setPhase(`${backend.toUpperCase()} · medición ${index + 1}/${MEASURED_RUNS}`)
        const tensor = new litert.Tensor(baseInput, shape)
        const started = performance.now()
        const outputs = await model.run(tensor)
      ...
      return summarizeRuns(backend, compileMs, warmups, runs)
    } catch (caught) {
  ...
  async function runBenchmark() {
    ...
    try {
      ...
      if (webgpu.supported && wasm.supported) {
        const delta = Math.abs(webgpu.medianMs - wasm.medianMs)
        if (delta < 0.2) winner = "tie"
        else winner = webgpu.medianMs < wasm.medianMs ? "webgpu" : "wasm"
        const faster = Math.min(webgpu.medianMs, wasm.medianMs)
        const slower = Math.max(webgpu.medianMs, wasm.medianMs)
      ...
      for (const result of [webgpu, wasm]) {
        ...
        recordLocalAIEvent({
          ...
          latencyMs: result.medianMs,
          compileMs: result.compileMs,
          runCount: result.runs.length,
          runtimeReused: runtime.reused,
          success: true,
          note: `p95 ${formatMs(result.p95Ms)} · CV ${result.cvPercent.toFixed(1)}%`,
        })
      }
      setPhase("Benchmark estadístico completado")
    } catch (caught) {
  ...
  return (
    <section className="rounded-[30px] border border-cyan-400/15 bg-[linear-gradient(180deg,rgba(4,13,29,0.99),rgba(3,8,20,0.99))] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-cyan-300"><Gauge className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.22em]">Performance Lab · statistical v3</p></div>
          <h2 className="mt-2 text-2xl font-black text-white">Benchmark WebGPU vs WASM</h2>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">Ejecuta {WARMUP_RUNS} warm-ups descartados y {MEASURED_RUNS} mediciones por backend. La decisión usa mediana y reporta media, p95, desviación y coeficiente de variación.</p>
        </div>
        <div className="flex gap-2">
          {history.length ? <button type="button" onClick={clearHistory} disabled={running} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-slate-950/55 px-3 py-2 text-xs font-black text-slate-400"><Trash2 className="h-3.5 w-3.5" /> Limpiar</button> : null}
          <button type="button" onClick={() => void runBenchmark()} disabled={running} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-950/25 px-4 py-2 text-sm font-black text-cyan-100 disabled:opacity-50">{running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}{running ? phase || "Midiendo…" : latest ? "Repetir benchmark" : "Ejecutar benchmark v3"}</button>
        </div>
      </div>
        ...
        <>
          <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_0.75fr]">
            {[latest.webgpu, latest.wasm].map(result => {
              ...
              return (
                <article key={result.backend} className={`rounded-[24px] border p-4 ${isWinner ? "border-emerald-400/25 bg-emerald-950/20" : "border-white/10 bg-slate-950/55"}`}>
                  ...
                  <p className="mt-3 text-3xl font-black text-white">{result.supported ? formatMs(result.medianMs) : "N/D"}</p>
                  <p className="mt-1 text-[10px] uppercase tracking-wider text-slate-600">mediana · {MEASURED_RUNS} corridas</p>
                  <div className="mt-3"><Sparkline values={result.runs} max={graphMax} /></div>
                  <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] sm:grid-cols-3">
                    {[
                      ["Media", formatMs(result.averageMs)],
                      ["P95", formatMs(result.p95Ms)],
                      ["Mín", formatMs(result.minMs)],
                      ["Máx", formatMs(result.maxMs)],
                      ["Desv.", formatMs(result.stdDevMs)],
                      ["CV", result.supported ? `${result.cvPercent.toFixed(1)}%` : "—"],
                    ].map(([label, value]) => <div key={label} className="rounded-xl bg-black/25 p-2"><p className="text-slate-600">{label}</p><p className="mt-1 font-black text-slate-300">{value}</p></div>)}
                  </div>
                  <div className="mt-2 rounded-xl bg-black/20 p-2 text-[10px] text-slate-500">Carga + compilación: <span className="font-black text-slate-300">{formatMs(result.compileMs)}</span> · {WARMUP_RUNS} warm-ups descartados</div>
                  {!result.supported ? <p className="mt-3 text-xs text-amber-300">{result.error}</p> : null}
                </article>
              )
            })}

            <article className="rounded-[24px] border border-violet-400/15 bg-violet-950/20 p-4">
              <div className="flex items-center gap-2 text-violet-200"><Trophy className="h-4 w-4" /><span className="text-xs font-black uppercase">Resultado</span></div>
              <p className="mt-4 text-2xl font-black text-white">{winnerLabel}</p>
              <p className="mt-2 text-xs leading-relaxed text-slate-400">{latest.speedup ? `La mediana del backend ganador es ${latest.speedup.toFixed(2)}× más rápida en este dispositivo.` : "No hay dos backends válidos para calcular aceleración."}</p>
              <div className="mt-4 rounded-2xl border border-white/5 bg-black/25 p-3 text-[10px] leading-5 text-slate-500">
                <p>Runtime: <span className="font-black text-slate-300">{latest.runtimeReused ? "reutilizado" : "inicializado"}</span></p>
                <p>Adquisición: <span className="font-black text-slate-300">{formatMs(latest.runtimeAcquireMs)}</span></p>
                <p>CPU threads: <span className="font-black text-slate-300">{latest.threads || "—"}</span></p>
                <p>Decisión: <span className="font-black text-slate-300">mediana, no mínimo</span></p>
              </div>
            </article>
          </div>

          <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/45 p-4">
            <p className="text-xs font-black text-white">Comparación visual · menor mediana es mejor</p>
            <div className="mt-4 space-y-3">
              {[latest.webgpu, latest.wasm].map(result => {
                const maxMedian = Math.max(latest.webgpu.medianMs || 0, latest.wasm.medianMs || 0, 1)
                return <div key={result.backend}><div className="mb-1 flex justify-between text-[10px]"><span className="font-black text-slate-500">{result.backend.toUpperCase()}</span><span className="font-black text-slate-300">{formatMs(result.medianMs)}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-900"><div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-emerald-400" style={{ width: `${Math.max(3, (result.medianMs / maxMedian) * 100)}%` }} /></div></div>
              })}
            </div>
          </div>
        </>
