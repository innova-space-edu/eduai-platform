...
export default function WhisperTinyLocalPanel() {
  ...
  const preparedRef = useRef<{ blob: Blob; data: WhisperAudioFeatures } | null>(null)
  ...
  async function preparedAudio() {
    ...
    if (preparedRef.current?.blob === audioBlob) return preparedRef.current.data
    setPhase("Decodificando audio y preparando log-Mel…")
    const data = await prepareWhisperAudio(audioBlob)
    preparedRef.current = { blob: audioBlob, data }
    return data
  }
  ...
  async function transcribe() {
    ...
    try {
      ...
      setPhase("Transcripción local completada")
      recordLocalAIEvent({
        ...
        runCount: 1,
        success: true,
        note: `Whisper local · ${result.decodedTokens} tokens · ${result.cacheSource}${fallbackUsed ? " · fallback local" : ""}`,
      })
    } catch (error) {
    ...
    try {
      ...
      for (const backend of backends) {
        try {
          ...
          results.push({ backend, supported: true, endToEndMs: result.modelEndToEndMs, acquireMs: result.acquireMs, text: result.text })
          recordLocalAIEvent({
            groupId,
            kind: "benchmark",
            backend,
            ...
            runCount: 1,
            success: true,
            note: "Whisper Router V3 calibration · one real-audio sample",
          })
        } catch (error) {
      ...
      const valid = results.filter(result => result.supported && typeof result.endToEndMs === "number").sort((a, b) => (a.endToEndMs || Infinity) - (b.endToEndMs || Infinity))
      if (valid.length) {
      ...
      setPhase("Calibración Whisper completada")
    } catch (error) {
  ...
  const prepared = preparedRef.current?.blob === audioBlob ? preparedRef.current.data : null
  ...
  return (
    <section className="mt-4 rounded-[30px] border border-sky-400/15 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.10),transparent_30%),linear-gradient(180deg,rgba(4,15,29,0.99),rgba(3,9,20,0.99))] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl"><div className="flex items-center gap-2 text-sky-300"><Waves className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.2em]">Whisper local · ASR v1</p></div><h2 className="mt-2 text-2xl font-black text-white">Whisper Tiny INT8 · voz a texto en el navegador</h2><p className="mt-2 text-sm leading-relaxed text-slate-400">Modelo ~{model?.sizeMB || 41.1} MB. Audio mono a 16 kHz, log-Mel de 80×3000, firmas encode/decode y tokenizer Whisper. Máximo {WHISPER_MAX_SECONDS} s por prueba.</p></div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
        <div className="rounded-[24px] border border-white/10 bg-slate-950/50 p-4">
          ...
          {audioBlob ? <div className="mt-4 rounded-2xl border border-white/5 bg-black/20 p-3"><div className="flex items-center gap-2 text-xs text-slate-300"><FileAudio className="h-4 w-4 text-sky-300" /><span className="font-black">Audio listo</span></div>{audioUrl ? <audio controls src={audioUrl} className="mt-3 w-full" /> : null}<div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-xl bg-black/20 p-2"><span className="text-slate-600">Duración</span><p className="mt-1 font-black text-slate-300">{prepared ? formatSeconds(prepared.durationSeconds) : "se calcula al procesar"}</p></div><div className="rounded-xl bg-black/20 p-2"><span className="text-slate-600">Ruta V3</span><p className="mt-1 font-black text-slate-300">{routeProfile ? `${routeProfile.backend.toUpperCase()} · ${formatMs(routeProfile.medianEndToEndMs)}` : "sin calibrar"}</p></div></div></div> : <div className="mt-4 rounded-2xl border border-dashed border-white/10 p-5 text-center text-xs text-slate-600">Graba una frase o selecciona un archivo de audio.</div>}
        ...
        <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
          ...
          {run.status === "success" && run.result ? <><div className="mt-4 min-h-32 rounded-2xl border border-emerald-400/10 bg-emerald-950/15 p-4 text-sm leading-6 text-slate-100">{run.result.text || <span className="text-slate-500">Whisper no detectó texto en este audio.</span>}</div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{[
            ["Preproceso", formatMs(run.preprocessMs)], ["Modelo", formatMs(run.result.modelEndToEndMs)], ["Encoder", formatMs(run.result.encodeMs)], ["Decoder", formatMs(run.result.decodeMs)], ["Adquirir", formatMs(run.result.acquireMs)], ["Tokenizer", formatMs(run.result.tokenizerMs)], ["Tokens", `${run.result.decodedTokens}`], ["Total real", formatMs(run.totalMs)],
          ].map(([label, value]) => <div key={label} className="rounded-xl bg-slate-950/55 p-2.5"><p className="text-[9px] uppercase text-slate-600">{label}</p><p className="mt-1 text-xs font-black text-white">{value}</p></div>)}</div>{run.fallbackUsed ? <p className="mt-3 rounded-xl border border-amber-400/15 bg-amber-950/20 p-3 text-[10px] text-amber-100">El backend preferido falló; EduAI usó el otro backend local. El audio no salió del navegador.</p> : null}<details className="mt-3 rounded-xl border border-white/5 bg-slate-950/40 p-3 text-[10px] text-slate-500"><summary className="cursor-pointer font-black text-slate-300">Firmas LiteRT detectadas</summary><div className="mt-2 space-y-1"><p>encode in: {run.result.encodeInputs.map(item => `${item.name} [${item.shape.join("×")}] ${item.dtype}`).join(" · ")}</p><p>encode out: {run.result.encodeOutputs.map(item => `${item.name} [${item.shape.join("×")}] ${item.dtype}`).join(" · ")}</p><p>decode in: {run.result.decodeInputs.map(item => `${item.name} [${item.shape.join("×")}] ${item.dtype}`).join(" · ")}</p><p>decode out: {run.result.decodeOutputs.map(item => `${item.name} [${item.shape.join("×")}] ${item.dtype}`).join(" · ")}</p></div></details></> : run.status === "error" ? <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-950/20 p-4 text-xs leading-5 text-amber-100">{run.error}</div> : <div className="mt-4 grid min-h-52 place-items-center rounded-2xl border border-dashed border-white/10 text-center text-xs text-slate-600">La transcripción aparecerá aquí.</div>}
        </div>
      </div>

      {calibration.length ? <div className="mt-4 rounded-[24px] border border-violet-400/15 bg-violet-950/15 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black text-violet-100">Calibración Router V3 · Whisper</p><p className="mt-1 text-[10px] text-slate-500">Una muestra E2E de audio real por backend. Repetir permite recalibrar.</p></div>{routeProfile ? <span className="rounded-full bg-emerald-950/35 px-3 py-1 text-[10px] font-black text-emerald-200">Ganador: {routeProfile.backend.toUpperCase()}</span> : null}</div><div className="mt-3 grid gap-2 sm:grid-cols-2">{calibration.map(item => <div key={item.backend} className="rounded-2xl border border-white/5 bg-black/20 p-3"><div className="flex items-center justify-between"><span className="text-xs font-black text-white">{item.backend.toUpperCase()}</span><span className={`text-[10px] font-black ${item.supported ? "text-emerald-300" : "text-amber-300"}`}>{item.supported ? formatMs(item.endToEndMs) : "No compatible"}</span></div>{item.supported ? <p className="mt-1 text-[10px] text-slate-500">Adquisición {formatMs(item.acquireMs)}</p> : <p className="mt-2 text-[10px] leading-4 text-slate-500">{item.error}</p>}</div>)}</div></div> : null}
    </section>
  )
}
