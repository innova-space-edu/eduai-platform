    ...
    try {
      ...
      recordLocalAIEvent({
        ...
        note: isLongFormResult(result)
          ? `Whisper long-form · ${result.chunks.length} bloques · ${result.decodedTokens} tokens · RTF ${result.realTimeFactor.toFixed(2)} · idioma ${result.language} · ${result.task}${fallbackUsed ? " · fallback local" : ""}`
          : `Whisper local · ${result.decodedTokens} tokens · ${result.tokensPerSecond.toFixed(1)} tok/s · idioma ${result.language} · ${result.task}${fallbackUsed ? " · fallback local" : ""}`,
      })
    } catch (error) {
      setProgress(null)
      setLongProgress(null)
      ...
      for (const backend of backends) {
        ...
        try {
          ...
          recordLocalAIEvent({
            ...
            note: textValid ? `Whisper Router V3 calibration · ${result.language} · ${result.task}` : "Whisper calibration descartada: sin texto válido",
          })
        } catch (error) {
    ...
    <section className="mt-4 rounded-[30px] border border-sky-400/15 bg-[radial-gradient(circle_at_top_right,rgba(14,165,233,0.10),transparent_30%),linear-gradient(180deg,rgba(4,15,29,0.99),rgba(3,9,20,0.99))] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2 text-sky-300"><Waves className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[0.2em]">Whisper local · ASR v1.3</p></div>
          <h2 className="mt-2 text-2xl font-black text-white">Whisper Tiny INT8 · voz a texto multilingüe</h2>
          <p className="mt-2 text-sm leading-relaxed text-slate-400">Audio corto o archivo completo por bloques de 30 s con {WHISPER_LONGFORM_OVERLAP_SECONDS} s de solapamiento, idioma automático/manual y timestamps globales. Todo permanece en el navegador.</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[0.8fr_1.2fr]">
        <div className="rounded-[24px] border border-white/10 bg-slate-950/50 p-4">
          ...
          {audioBlob ? <div className="mt-4 rounded-2xl border border-white/5 bg-black/20 p-3">
            ...
            {sourceDuration > WHISPER_MAX_SECONDS ? <div className="mt-3 rounded-xl border border-cyan-400/15 bg-cyan-950/10 p-3">
              ...
              {fullAudio ? <p className="mt-2 text-[9px] leading-4 text-cyan-100/60">EduAI procesará todo el archivo en {chunkCount} ventanas con {WHISPER_LONGFORM_OVERLAP_SECONDS} s de solapamiento y unirá el texto automáticamente.</p> : <><div className="mt-3 flex items-center justify-between"><p className="text-[10px] font-black text-amber-100">Tramo: {segmentStartSeconds.toFixed(1)}–{segmentEndPreview.toFixed(1)} s</p><div className="flex gap-1"><button type="button" onClick={() => updateSegmentStart(0)} className="rounded border border-white/10 px-2 py-1 text-[9px] text-slate-300">Inicio</button><button type="button" onClick={() => updateSegmentStart(maxSegmentStart)} className="rounded border border-white/10 px-2 py-1 text-[9px] text-slate-300">Final</button></div></div><input type="range" min={0} max={maxSegmentStart} step={0.5} value={Math.min(segmentStartSeconds, maxSegmentStart)} onChange={event => updateSegmentStart(Number(event.target.value))} className="mt-2 w-full accent-amber-400" /></>}
        ...
        <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
          ...
          {run.status === "success" && result ? <>
            ...
            {longResult ? <details className="mt-3 rounded-xl border border-white/5 bg-slate-950/40 p-3 text-[10px] text-slate-500" open><summary className="cursor-pointer font-black text-slate-300">Bloques transcritos · timestamps globales</summary><div className="mt-3 space-y-2">{longResult.chunks.map(chunk => <div key={chunk.index} className="rounded-xl border border-white/5 bg-black/20 p-3"><div className="flex items-center justify-between gap-2"><span className="font-black text-slate-300">Bloque {chunk.index + 1}/{longResult.chunks.length} · {chunk.startSeconds.toFixed(1)}–{chunk.endSeconds.toFixed(1)} s</span><span>{formatMs(chunk.result.modelEndToEndMs)} · {chunk.result.decodedTokens} tokens</span></div><p className="mt-2 leading-4 text-slate-400">{chunk.result.text || "Sin texto"}</p></div>)}</div></details> : singleResult ? <details className="mt-3 rounded-xl border border-white/5 bg-slate-950/40 p-3 text-[10px] text-slate-500"><summary className="cursor-pointer font-black text-slate-300">Diagnóstico técnico · tokens y timestamps</summary><div className="mt-2 space-y-1"><p>prompt: {singleResult.prefixTokenIds.join(", ")}</p><p>texto técnico: {singleResult.rawText || "—"}</p><p>token ids: {singleResult.tokenIds.slice(0, 20).join(", ") || "ninguno"}</p></div></details> : null}
