    ...
    <section className="rounded-[30px] border border-cyan-400/15 bg-[linear-gradient(180deg,rgba(4,13,29,0.99),rgba(3,8,20,0.99))] p-5 sm:p-6">
      ...
      {latest ? <>
        <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr_0.75fr]">
          ...
          <article className="rounded-[24px] border border-violet-400/15 bg-violet-950/20 p-4"><div className="flex items-center gap-2 text-violet-200"><Trophy className="h-4 w-4" /><span className="text-xs font-black uppercase">Router V2</span></div><p className="mt-4 text-2xl font-black text-white">{winnerLabel}</p><p className="mt-2 text-xs leading-relaxed text-slate-400">{latest.speedup ? `La mediana end-to-end del ganador es ${latest.speedup.toFixed(2)}× más rápida en este dispositivo.` : "No hay dos backends válidos para calcular aceleración."}</p><div className="mt-4 rounded-2xl border border-white/5 bg-black/25 p-3 text-[10px] leading-5 text-slate-500"><p>Perfil guardado: <span className="font-black text-slate-300">{latest.routeSaved ? "sí" : "no"}</span></p><p>Runtime: <span className="font-black text-slate-300">{latest.runtimeReused ? "reutilizado" : "inicializado"}</span></p><p>Adquisición: <span className="font-black text-slate-300">{formatMs(latest.runtimeAcquireMs)}</span></p><p>CPU threads: <span className="font-black text-slate-300">{latest.threads || "—"}</span></p></div></article>
        </div>
