    ...
    for (const track of project.tracks.filter((item) => item.kind === "audio" || item.kind === "music")) {
      for (const clip of track.clips) {
        ...
        if (!audio) {
          ...
          audio.preload = "auto";
          audio.crossOrigin = asset.url.startsWith("http") ? "anonymous" : "";
    ...
    <main className="min-h-[100dvh] bg-[#050816] text-slate-100">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#07101f]/95 backdrop-blur-xl">
        <div className="flex min-h-16 items-center gap-3 px-4 lg:px-6">
          <Link href="/agentes" className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/5 hover:bg-white/10" title="Volver a Agentes">
            <ArrowLeft size={18} />
          </Link>
          <div className="min-w-0 flex-1">
            ...
            <input
              value={project.title}
              onChange={(event) => setProject((current) => ({ ...current, title: event.target.value, updatedAt: new Date().toISOString() }))}
              className="w-full max-w-xl bg-transparent text-lg font-bold outline-none placeholder:text-slate-500"
          ...
          <button onClick={() => downloadJson(project, assets)} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold hover:bg-white/10">
            <Save size={15} /> Guardar
          </button>
          <button onClick={exportWebM} disabled={exporting} className="flex items-center gap-2 rounded-xl bg-cyan-500 px-3 py-2 text-xs font-bold text-slate-950 hover:bg-cyan-400 disabled:opacity-50">
            <Download size={15} /> {exporting ? `${Math.round(exportProgress * 100)}%` : "Exportar WebM"}
          </button>
        </div>
      </header>
        ...
        <aside className="border-r border-white/10 bg-[#07101f] p-3">
          <div className="mb-3 grid grid-cols-3 gap-1 rounded-xl bg-black/20 p-1">
            {([
              ...
              ["music", "Música", Music2],
              ["project", "Proyecto", Film],
            ] as const).map(([value, label, Icon]) => (
              <button key={value} onClick={() => setTab(value)} className={`flex items-center justify-center gap-1 rounded-lg px-2 py-2 text-[11px] font-semibold ${tab === value ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"}`}>
                <Icon size={14} /> {label}
              </button>
            ))}
          </div>
          ...
          {tab === "files" && (
            <div className="space-y-3">
              <label className="flex cursor-pointer items-center justify-center gap-2 rounded-2xl border border-dashed border-cyan-400/40 bg-cyan-500/5 px-4 py-5 text-sm font-semibold text-cyan-200 hover:bg-cyan-500/10">
                <Upload size={18} /> Cargar audio, video o imagen
                  ...
                  <div key={asset.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5">
                    <div className="flex items-center gap-2">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/5">
                        {asset.kind === "video" ? <Video size={17} /> : asset.kind === "audio" ? <AudioLines size={17} /> : <ImageIcon size={17} />}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-xs font-semibold">{asset.name}</p>
                        <p className="text-[10px] text-slate-500">{asset.kind} · {fmt(asset.duration)}</p>
                      </div>
          ...
          {tab === "music" && (
            <div className="space-y-3">
              <div className="flex gap-2">
                <input value={musicQuery} onChange={(event) => setMusicQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && searchMusic()} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs outline-none focus:border-emerald-400/50" placeholder="Buscar canción, ambiente…" />
                <button onClick={searchMusic} disabled={searchingMusic} className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500 text-slate-950 disabled:opacity-50"><Search size={16} /></button>
              </div>
              ...
              <div className="space-y-2">
                {musicResults.map((result) => (
                  <div key={result.id} className="flex gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2">
                    <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-emerald-500/10">
                      {result.artworkUrl ? <img src={result.artworkUrl} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><Music2 size={16} /></div>}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[11px] font-semibold">{result.title}</p>
                      <p className="truncate text-[10px] text-slate-500">{result.artist} · {result.source}</p>
                    </div>
            ...
            <div className="space-y-3 text-xs">
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-slate-400">
                  <span>{project.width} × {project.height}</span><span>{project.fps} fps</span>
                  <span>{clips.length} clips</span><span>{fmt(duration)}</span>
                </div>
              </div>
        ...
        <section className="flex min-w-0 flex-col bg-[#050816]">
          <div className="flex min-h-[430px] flex-1 items-center justify-center p-4 lg:p-6">
            <div className="relative aspect-video w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl shadow-cyan-950/20">
              {!activeVideoAsset && !activeOverlayAsset && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-slate-600">
                  ...
                  <div className="text-center"><p className="text-sm font-semibold text-slate-400">Previsualización</p><p className="mt-1 text-xs">Agrega un video o imagen a la línea de tiempo.</p></div>
                </div>
              )}
              {activeVideoAsset?.url && (
                <video key={activeVideoAsset.id} ref={videoRef} src={activeVideoAsset.url} className="absolute inset-0 h-full w-full object-cover" playsInline />
              )}
              {activeOverlayAsset?.url && (
                <img src={activeOverlayAsset.url} alt="Overlay" className="absolute inset-0 h-full w-full object-cover" style={{ opacity: activeOverlayClip?.opacity ?? 1 }} />
              )}
            </div>
          </div>
          ...
          <div className="border-t border-white/10 bg-[#07101f]">
            <div className="flex items-center gap-3 border-b border-white/10 px-3 py-2">
              <button onClick={() => { if (playhead >= duration) setPlayhead(0); setPlaying((value) => !value); }} className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-500 text-slate-950">{playing ? <Pause size={16} /> : <Play size={16} />}</button>
              <span className="w-24 font-mono text-xs text-cyan-200">{fmt(playhead)}</span>
              <input type="range" min={0} max={duration} step={0.05} value={Math.min(playhead, duration)} onChange={(event) => { setPlaying(false); setPlayhead(Number(event.target.value)); }} className="min-w-0 flex-1 accent-cyan-400" />
              <span className="font-mono text-xs text-slate-500">{fmt(duration)}</span>
            </div>
            <div className="max-h-[330px] overflow-auto">
              <div style={{ width: timelineWidth + 118 }} className="min-w-full p-2">
                <div className="ml-[110px] mb-1 flex h-5 items-end border-b border-white/10 text-[9px] text-slate-600" style={{ width: timelineWidth }}>
                  {Array.from({ length: Math.ceil(duration / 5) + 1 }, (_, index) => <span key={index} className="absolute" style={{ marginLeft: Math.min(timelineWidth - 20, index * 5 * 64) }}>{index * 5}s</span>)}
                </div>
                {project.tracks.map((track) => (
                  <div key={track.id} className="mb-1 flex h-14">
                    <div className="flex w-[110px] shrink-0 items-center gap-2 border-r border-white/10 px-2 text-[10px] font-semibold text-slate-400">
                      {track.kind === "video" ? <Video size={14} /> : track.kind === "overlay" ? <ImageIcon size={14} /> : track.kind === "music" ? <Music2 size={14} /> : <AudioLines size={14} />}
                      <span className="truncate">{track.name}</span>
                    </div>
                    <div
                      className="relative h-full bg-black/20"
                      style={{ width: timelineWidth }}
                      onClick={(event) => {
                        const rect = event.currentTarget.getBoundingClientRect();
                        const x = event.clientX - rect.left;
                        setPlaying(false);
                        setPlayhead(clamp((x / timelineWidth) * duration, 0, duration));
                      }}
                    >
                      {track.clips.map((clip) => {
                        ...
                        return (
                          <button
                            key={clip.id}
                            onClick={(event) => { event.stopPropagation(); setSelectedClipId(clip.id); }}
                            className={`absolute top-1 h-12 overflow-hidden rounded-lg border px-2 text-left text-[10px] ${TRACK_ACCENTS[track.kind]} ${selectedClipId === clip.id ? "ring-2 ring-white/70" : ""}`}
                            style={{ left: (clip.start / duration) * timelineWidth, width: Math.max(24, (clip.duration / duration) * timelineWidth) }}
                            title={`${asset?.name || "Clip"} · ${fmt(clip.start)}–${fmt(clip.start + clip.duration)}`}
                          >
                            <span className="block truncate font-semibold">{asset?.name || "Clip"}</span>
                            <span className="block opacity-60">{fmt(clip.duration)}</span>
                          </button>
                        );
                      })}
        ...
        <aside className="border-l border-white/10 bg-[#07101f] p-3">
          ...
          {selectedClip && selectedAsset ? (
            <div className="space-y-4">
              <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                <p className="truncate text-sm font-semibold">{selectedAsset.name}</p>
                <p className="mt-1 text-[10px] uppercase tracking-wider text-slate-500">{selectedAsset.kind} · {selectedAsset.source || "local"}</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <button onClick={splitClip} className="flex items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-2 py-2 text-[11px] hover:bg-white/10"><Scissors size={14} /> Dividir</button>
                <button onClick={duplicateClip} className="flex items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-2 py-2 text-[11px] hover:bg-white/10"><Copy size={14} /> Duplicar</button>
              </div>
              <div className="space-y-3 rounded-xl border border-white/10 bg-white/[0.03] p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="block text-[10px] text-slate-400">Duración<input type="number" min={0.2} step={0.1} value={selectedClip.duration} onChange={(event) => updateClip(selectedClip.id, { duration: Math.max(0.2, Number(event.target.value) || 0.2) })} className="mt-1 w-full rounded-lg border border-white/10 bg-black/20 px-2 py-2 text-xs text-white outline-none" /></label>
                  <label className="block text-[10px] text-slate-400">Recorte inicio<input type="number" min={0} step={0.1} value={selectedClip.offset} onChange={(event) => updateClip(selectedClip.id, { offset: Math.max(0, Number(event.target.value) || 0) })} className="mt-1 w-full rounded-lg border border-white/10 bg-black/20 px-2 py-2 text-xs text-white outline-none" /></label>
                </div>
                <div>
                  <div className="mb-1 flex items-center justify-between text-[10px] text-slate-400"><span className="flex items-center gap-1"><Volume2 size={12} /> Volumen</span><span>{Math.round(selectedClip.volume * 100)}%</span></div>
                  <input type="range" min={0} max={1} step={0.01} value={selectedClip.volume} onChange={(event) => updateClip(selectedClip.id, { volume: Number(event.target.value) })} className="w-full accent-violet-400" />
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => updateClip(selectedClip.id, { start: Math.max(0, selectedClip.start - 0.5) })} className="rounded-xl border border-white/10 py-2 text-xs hover:bg-white/5">← 0,5 s</button>
                <button onClick={() => updateClip(selectedClip.id, { start: selectedClip.start + 0.5 })} className="rounded-xl border border-white/10 py-2 text-xs hover:bg-white/5">0,5 s →</button>
              </div>
          ...
          <div className="mt-5 rounded-xl border border-cyan-400/15 bg-cyan-500/5 p-3 text-[10px] leading-relaxed text-cyan-100/60">
            <p className="font-semibold text-cyan-200">Estado del editor</p>
