...
import { ArrowLeft, CheckCircle2, ChevronLeft, ChevronRight, Eye, FolderOpen, Info, LoaderCircle, RotateCcw, Sparkles, Upload, WandSparkles } from "lucide-react"
import { FlashcardsContentEditor, QuizContentEditor } from "@/components/creator-hub/EditableStudyEditors"
import { EditableFlashcardPreview, EditableQuizQuestionPreview } from "@/components/creator-hub/EditableStudyPreviews"
...
type StudyFormat = "flashcards" | "quiz"
...
const CONFIG = {
  flashcards: {
    title: "Edita tu mazo de estudio",
    description: "Corrige preguntas, respuestas, pistas, reglas mnemónicas y niveles de dificultad.",
    processing: ["Extrayendo conceptos", "Creando preguntas y respuestas", "Ordenando el mazo"],
    answerLabel: "Voltear tarjeta",
  },
  quiz: {
    title: "Edita la evaluación",
    description: "Ajusta enunciados, distractores, respuestas correctas y retroalimentación.",
    processing: ["Analizando aprendizajes", "Diseñando distractores", "Construyendo la evaluación"],
    answerLabel: "Ver solución",
  },
} as const

function getTitle(format: StudyFormat, data: any, fallback: string) {
  return format === "flashcards" ? data?.deckTitle || data?.topic || fallback : data?.title || data?.topic || fallback
}

function getPalette(data: any) {
...
export default function EditableStudyCreatorPage({ format }: { format: StudyFormat }) {
  const meta = getCreatorHubFormat(format)
  const config = CONFIG[format]
  const [sourceType, setSourceType] = useState<SourceType>("topic")
  const [content, setContent] = useState("")
  const [fileName, setFileName] = useState("")
  const [accentColor, setAccentColor] = useState(meta?.color || "#22c55e")
  ...
  useEffect(() => {
    ...
    if (previewIndex >= items.length) setPreviewIndex(items.length - 1)
  }, [items.length, previewIndex])

  useEffect(() => setShowAnswer(false), [previewIndex])
  ...
  const reset = () => {
    ...
    setPreviewIndex(0)
    setShowAnswer(false)
    setAccentColor(meta?.color || "#22c55e")
  ...
  const generate = async () => {
    ...
    try {
      ...
      const project = saveCreatorHubProject({
        format,
        title: getTitle(format, generated, meta?.label || "Material"),
        data: generated,
        accentColor,
        designTemplateId,
      })
  ...
  const updateResult = (next: any) => {
    ...
    const updated = updateCreatorHubProject(projectId, {
      title: getTitle(format, next, meta?.label || "Material"),
      data: next,
      accentColor,
      designTemplateId,
    })
  ...
  const Editor = format === "flashcards" ? FlashcardsContentEditor : QuizContentEditor
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-5 py-3.5 sm:px-7">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden truncate text-[11px] text-muted2 sm:block">Genera, modifica y guarda materiales de estudio.</p>
            </div>
          </div>
      ...
      <main className="mx-auto grid max-w-[1600px] items-start gap-5 px-5 py-6 sm:px-7 xl:grid-cols-[420px_minmax(0,1fr)]">
          ...
          {step !== "result" ? (
            <>
              <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-muted2">Parte desde un tema o una fuente y luego edita cada elemento generado.</p>
                ...
                <div className="mt-4">
                  ...
                  {(sourceType === "topic" || sourceType === "text" || sourceType === "url") ? (
                    <textarea value={content} onChange={(event) => setContent(event.target.value)} rows={sourceType === "text" ? 9 : 5} placeholder={sourceType === "topic" ? meta.placeholder : sourceType === "url" ? "https://ejemplo.com/articulo" : "Pega aquí el contenido..."} className="mt-2 w-full resize-y rounded-2xl border border-soft bg-card-soft-theme px-3.5 py-3 text-sm text-main outline-none placeholder:text-muted2 focus:border-blue-500/30 focus:bg-input-theme" />
                  ) : (
              ...
              <button type="button" onClick={generate} disabled={!content.trim() || processing} className="flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3.5 text-sm font-bold text-white disabled:opacity-35" style={{ background: `linear-gradient(135deg,${meta.color}cc,${meta.color})`, boxShadow: content.trim() ? `0 12px 24px ${meta.color}24` : "none" }}>{processing ? <LoaderCircle size={16} className="animate-spin" /> : <Sparkles size={16} />}{processing ? "Generando versión editable..." : `Generar ${meta.label}`}</button>
            </>
          ) : (
            <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
              <div className="mb-4"><p className="text-[10px] font-black uppercase tracking-[0.16em]" style={{ color: meta.color }}>Edición activa</p><h2 className="mt-1 text-base font-bold text-main">{config.title}</h2><p className="mt-1 text-xs leading-relaxed text-muted2">{config.description}</p></div>
          ...
          {step === "result" && result && (
            <div className="space-y-4 p-4 sm:p-5">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-soft bg-card-soft-theme px-3 py-2">
                ...
                <button type="button" onClick={() => setShowAnswer((value) => !value)} className="inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold" style={{ borderColor: `${meta.color}35`, background: `${meta.color}0d`, color: meta.color }}><Eye size={13} /> {showAnswer ? "Ocultar respuesta" : config.answerLabel}</button>
                <button type="button" onClick={() => setPreviewIndex((index) => Math.min(items.length - 1, index + 1))} disabled={previewIndex >= items.length - 1} className="inline-flex items-center gap-1 rounded-xl border border-soft px-3 py-1.5 text-xs font-bold text-muted2 disabled:opacity-30">Siguiente <ChevronRight size={13} /></button>
              </div>
              ...
              <div id="creator-result-container" className="overflow-auto rounded-2xl border p-4 sm:p-5" style={{ background: palette.background || "var(--bg-card-soft)", borderColor: palette.primary ? `${palette.primary}22` : "var(--border-soft)" }}>
                {format === "flashcards" ? <EditableFlashcardPreview data={result} index={previewIndex} showBack={showAnswer} /> : <EditableQuizQuestionPreview data={result} index={previewIndex} showSolution={showAnswer} />}
              </div>

              <CreatorHubUtilityBar format={format} data={result} accentColor={accentColor} designTemplateId={designTemplateId} title={getTitle(format, result, meta.label)} />
            </div>
          )}
        </section>
      </main>
