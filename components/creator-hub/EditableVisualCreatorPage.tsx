...
const CONFIG = {
  infographic: {
    editorTitle: "Edita cada bloque de la infografía",
    editorDescription: "Puedes cambiar títulos, cifras, ideas, íconos, secciones y conclusión sin volver a generar.",
    processing: ["Analizando la fuente", "Jerarquizando información", "Construyendo bloques visuales"],
  },
  ppt: {
    editorTitle: "Edita la presentación diapositiva por diapositiva",
    editorDescription: "Reordena, duplica, elimina y modifica el contenido antes de descargar el PPTX final.",
    processing: ["Analizando la fuente", "Diseñando la secuencia pedagógica", "Construyendo diapositivas"],
  },
} as const
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-5 py-3.5 sm:px-7">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden truncate text-[11px] text-muted2 sm:block">Genera, modifica y exporta desde un mismo espacio.</p>
            </div>
          </div>
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            {(step === "result" || step === "processing") && (
              <button type="button" onClick={reset} disabled={processing} className="inline-flex items-center gap-1.5 rounded-xl border border-soft px-3 py-2 text-xs font-bold text-muted2 transition hover:bg-card-soft-theme hover:text-main disabled:opacity-40">
      ...
      <main className="mx-auto grid max-w-[1600px] items-start gap-5 px-5 py-6 sm:px-7 xl:grid-cols-[420px_minmax(0,1fr)]">
          ...
          {step !== "result" ? (
            <>
              <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-muted2">Selecciona una fuente y deja que EduAI construya una primera versión completamente editable.</p>
              ...
              <section className="space-y-5 rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
                <div>
                  ...
                  <div className="mt-2"><TemplatePicker format={format} value={designTemplateId} onChange={(templateId, nextAccentColor) => { setDesignTemplateId(templateId); if (nextAccentColor) setAccentColor(nextAccentColor) }} compact /></div>
                </div>
                <ColorPalette value={accentColor} onChange={setAccentColor} />
              </section>

              <button type="button" onClick={generate} disabled={!content.trim() || processing} className="flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3.5 text-sm font-bold text-white transition-all disabled:opacity-35" style={{ background: `linear-gradient(135deg,${meta.color}cc,${meta.color})`, boxShadow: content.trim() ? `0 12px 24px ${meta.color}24` : "none" }}>
                {processing ? <LoaderCircle size={16} className="animate-spin" /> : <Sparkles size={16} />}
                {processing ? "Generando versión editable..." : `Generar ${meta.label}`}
              </button>
            </>
          ) : (
            <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
              <div className="mb-4">
                <p className="text-[10px] font-black uppercase tracking-[0.16em]" style={{ color: meta.color }}>Edición activa</p>
                <h2 className="mt-1 text-base font-bold text-main">{config.editorTitle}</h2>
                <p className="mt-1 text-xs leading-relaxed text-muted2">{config.editorDescription}</p>
              </div>
        ...
        <section className="min-w-0 overflow-hidden rounded-3xl border border-soft bg-card-theme">
          <div className="flex items-center justify-between gap-3 border-b border-soft px-4 py-3.5 sm:px-5">
            <div>
              <p className="text-sm font-bold text-main">Vista de trabajo</p>
              <p className="mt-0.5 text-[11px] text-muted2">La previsualización y las exportaciones usan la versión más reciente.</p>
            </div>
          ...
          {step === "input" && (
            <div className="flex min-h-[650px] flex-col items-center justify-center px-6 py-10 text-center">
              ...
              <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted2">Después de generar podrás editar el contenido, reorganizar la estructura y descargar el material actualizado.</p>
              ...
              <div className="mt-5 flex max-w-2xl items-start gap-2 rounded-2xl border border-soft bg-card-soft-theme p-3.5 text-left">
                ...
                <p className="text-xs leading-relaxed text-muted2">Esta mejora se concentra en Creator Hub. No modifica Cuaderno EduAI, Notebook ni los agentes existentes.</p>
              </div>
            </div>
          )}
          ...
          {step === "processing" && (
            <div className="flex min-h-[650px] flex-col items-center justify-center px-6 py-10 text-center">
              ...
              <h2 className="mt-5 text-lg font-bold text-main">Preparando una versión editable...</h2>
              <p className="mt-2 max-w-md text-sm text-muted2">EduAI está organizando el contenido para que puedas modificarlo después de la generación.</p>
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                {config.processing.map((label) => <span key={label} className="animate-pulse rounded-full border border-soft bg-card-soft-theme px-3 py-1 text-[11px] text-muted2">{label}</span>)}
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 p-4 sm:p-5">
              <div className="flex flex-col gap-3 rounded-2xl border p-3.5 sm:flex-row sm:items-center" style={{ background: `${meta.color}08`, borderColor: `${meta.color}22` }}>
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold" style={{ color: meta.color }}>{meta.label} lista para editar</p>
                  <p className="mt-0.5 text-[11px] text-muted2">Edita desde el panel izquierdo. Las exportaciones se actualizarán automáticamente.</p>
                </div>
