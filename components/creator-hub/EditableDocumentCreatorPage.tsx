...
import { CornellContentEditor, GlossaryContentEditor, LessonPlanContentEditor, PosterContentEditor } from "@/components/creator-hub/EditableDocumentEditors"
import { EditableCornellPreview, EditableGlossaryPreview, EditableLessonPlanPreview, EditablePosterPreview } from "@/components/creator-hub/EditableDocumentPreviews"
...
export type DocumentFormat = "poster" | "cornell" | "glossary" | "lessonplan"
...
const CONFIG = {
  poster: {
    title: "Edita el afiche por bloques",
    description: "Ajusta la jerarquía, los mensajes, datos destacados, íconos y llamado a la acción.",
    processing: ["Sintetizando el mensaje", "Diseñando la jerarquía", "Construyendo bloques visuales"],
  },
  cornell: {
    title: "Edita los apuntes Cornell",
    description: "Corrige las preguntas clave, desarrolla las notas, ajusta palabras clave y mejora la síntesis.",
    processing: ["Identificando conceptos", "Organizando preguntas y notas", "Redactando la síntesis"],
  },
  glossary: {
    title: "Edita cada término del glosario",
    description: "Modifica definiciones, ejemplos, categorías y el orden de los conceptos.",
    processing: ["Extrayendo vocabulario", "Redactando definiciones", "Creando ejemplos contextualizados"],
  },
  lessonplan: {
    title: "Edita la planificación de clase",
    description: "Ajusta objetivo, evaluación, recursos y cada momento pedagógico antes de exportar.",
    processing: ["Analizando el aprendizaje", "Secuenciando actividades", "Alineando evaluación y recursos"],
  },
} as const

function resultTitle(format: DocumentFormat, data: any, fallback: string) {
  if (format === "poster") return data?.headline || fallback
  return data?.title || data?.subject || fallback
}
...
function DocumentEditor({ format, data, onChange }: { format: DocumentFormat; data: any; onChange: (next: any) => void }) {
  if (format === "poster") return <PosterContentEditor data={data} onChange={onChange} />
  if (format === "cornell") return <CornellContentEditor data={data} onChange={onChange} />
  if (format === "glossary") return <GlossaryContentEditor data={data} onChange={onChange} />
  return <LessonPlanContentEditor data={data} onChange={onChange} />
}

function DocumentPreview({ format, data, accentColor }: { format: DocumentFormat; data: any; accentColor: string }) {
  if (format === "poster") return <EditablePosterPreview data={data} accentColor={accentColor} />
  if (format === "cornell") return <EditableCornellPreview data={data} accentColor={accentColor} />
  if (format === "glossary") return <EditableGlossaryPreview data={data} accentColor={accentColor} />
  return <EditableLessonPlanPreview data={data} accentColor={accentColor} />
}

export default function EditableDocumentCreatorPage({ format }: { format: DocumentFormat }) {
      ...
      <header className="sticky top-0 z-30 border-b border-soft bg-header-theme backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-5 py-3.5 sm:px-7">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><p className="truncate text-sm font-bold text-main sm:text-base">{meta.label} editable</p><p className="hidden truncate text-[11px] text-muted2 sm:block">Genera, modifica y exporta un documento terminado.</p></div>
          </div>
          ...
          {step !== "result" ? (
            <>
              <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5">
                ...
                <p className="mt-1.5 text-xs leading-relaxed text-muted2">Selecciona una fuente. EduAI construirá una primera versión que podrás modificar por completo.</p>
              ...
              <section className="space-y-5 rounded-3xl border border-soft bg-card-theme p-4 sm:p-5"><div><label className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted2">3. Diseño visual</label><div className="mt-2"><TemplatePicker format={format} value={designTemplateId} onChange={(templateId, nextAccentColor) => { setDesignTemplateId(templateId); if (nextAccentColor) setAccentColor(nextAccentColor) }} compact /></div></div><ColorPalette value={accentColor} onChange={setAccentColor} /></section>
              <button type="button" onClick={generate} disabled={!content.trim() || processing} className="flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3.5 text-sm font-bold text-white disabled:opacity-35" style={{ background: `linear-gradient(135deg,${meta.color}cc,${meta.color})`, boxShadow: content.trim() ? `0 12px 24px ${meta.color}24` : "none" }}>{processing ? <LoaderCircle size={16} className="animate-spin" /> : <Sparkles size={16} />}{processing ? "Generando documento editable..." : `Generar ${meta.label}`}</button>
            </>
          ) : (
            <section className="rounded-3xl border border-soft bg-card-theme p-4 sm:p-5"><div className="mb-4"><p className="text-[10px] font-black uppercase tracking-[0.16em]" style={{ color: meta.color }}>Edición activa</p><h2 className="mt-1 text-base font-bold text-main">{config.title}</h2><p className="mt-1 text-xs leading-relaxed text-muted2">{config.description}</p></div><DocumentEditor format={format} data={result} onChange={updateResult} /></section>
          )}
        </aside>

        <section className="min-w-0 overflow-hidden rounded-3xl border border-soft bg-card-theme">
          <div className="flex items-center justify-between gap-3 border-b border-soft px-4 py-3.5 sm:px-5"><div><p className="text-sm font-bold text-main">Vista del documento</p><p className="mt-0.5 text-[11px] text-muted2">La exportación utiliza esta versión limpia.</p></div><span className="rounded-full px-2.5 py-1 text-[10px] font-bold" style={{ background: `${meta.color}12`, color: meta.color }}>{meta.icon} {meta.label}</span></div>
          {step === "input" && <div className="flex min-h-[680px] flex-col items-center justify-center px-6 py-10 text-center"><div className="flex h-20 w-20 items-center justify-center rounded-[28px] text-4xl" style={{ background: `${meta.color}12`, border: `1px solid ${meta.color}22` }}>{meta.icon}</div><h2 className="mt-5 text-lg font-bold text-main">Crea una primera versión con IA</h2><p className="mt-2 max-w-xl text-sm leading-relaxed text-muted2">Después podrás editar cada bloque sin perder el diseño ni volver a generar todo.</p><div className="mt-6 grid w-full max-w-2xl gap-2 sm:grid-cols-3">{meta.highlights.map((highlight) => <div key={highlight} className="rounded-2xl border border-soft bg-card-soft-theme p-3 text-xs text-sub">✓ {highlight}</div>)}</div><div className="mt-5 flex max-w-2xl items-start gap-2 rounded-2xl border border-soft bg-card-soft-theme p-3.5 text-left"><Info size={15} className="mt-0.5 flex-shrink-0 text-blue-500" /><p className="text-xs leading-relaxed text-muted2">Esta mejora está aislada en Creator Hub y no modifica Cuaderno EduAI ni los agentes existentes.</p></div></div>}
          {step === "processing" && <div className="flex min-h-[680px] flex-col items-center justify-center px-6 py-10 text-center"><div className="relative h-20 w-20"><div className="absolute inset-0 animate-spin rounded-full border-2 border-soft" style={{ borderTopColor: meta.color }} /><div className="absolute inset-0 flex items-center justify-center text-3xl">{meta.icon}</div></div><h2 className="mt-5 text-lg font-bold text-main">Preparando documento editable...</h2><p className="mt-2 max-w-md text-sm text-muted2">EduAI está organizando la información y su jerarquía visual.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{config.processing.map((label) => <span key={label} className="animate-pulse rounded-full border border-soft bg-card-soft-theme px-3 py-1 text-[11px] text-muted2">{label}</span>)}</div></div>}
          {step === "result" && result && <div className="space-y-4 p-4 sm:p-5"><div className="flex flex-col gap-3 rounded-2xl border p-3.5 sm:flex-row sm:items-center" style={{ background: `${meta.color}08`, borderColor: `${meta.color}22` }}><CheckCircle2 size={18} style={{ color: meta.color }} className="flex-shrink-0" /><div className="min-w-0 flex-1"><p className="text-sm font-bold" style={{ color: meta.color }}>{meta.label} listo para editar</p><p className="mt-0.5 text-[11px] text-muted2">Los cambios del panel izquierdo se reflejan inmediatamente.</p></div><Link href="/creator-hub/projects" className="flex items-center gap-1.5 text-xs font-bold text-sub hover:text-main"><FolderOpen size={13} /> Ver proyectos</Link></div><div id="creator-result-container" className="overflow-auto rounded-2xl border p-3 sm:p-5" style={{ background: palette.background || "var(--bg-card-soft)", borderColor: palette.primary ? `${palette.primary}22` : "var(--border-soft)" }}><DocumentPreview format={format} data={result} accentColor={accentColor} /></div><CreatorHubUtilityBar format={format} data={result} accentColor={accentColor} designTemplateId={designTemplateId} title={resultTitle(format, result, meta.label)} /></div>}
        </section>
      </main>
    </div>
  )
