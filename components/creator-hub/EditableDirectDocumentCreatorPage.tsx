...
import {
  ensureDirectDocumentCanvasData,
  type DirectDocumentFormat,
} from "@/lib/creator-document-canvas"
...
function titleFor(format: DirectDocumentFormat, data: any, fallback: string) {
  if (format === "lessonplan") return data?.title || fallback
...
export default function EditableDirectDocumentCreatorPage({ format }: { format: DirectDocumentFormat }) {
  ...
  useEffect(() => {
    ...
    void loadCloudCreatorHubProject(requestedProjectId).then(async (project) => {
      ...
      let prepared = ensureDirectDocumentCanvasData(
        project.data,
        format,
        color,
  ...
  const persist = (
    ...
    const updated = updateCreatorHubProject(projectId, {
      title: titleFor(format, prepared, meta?.label || "Documento"),
      data: prepared,
  ...
  const changeTemplate = (
    ...
    let next = ensureDirectDocumentCanvasData(
      result,
      format,
      color,
  ...
  const generate = async () => {
    ...
    try {
      ...
      let generated = ensureDirectDocumentCanvasData(
        payload.output.data,
        format,
        accentColor,
      ...
      const project = saveCreatorHubProject({
        format,
        title: titleFor(format, generated, meta?.label || "Documento"),
        data: generated,
        accentColor,
        designTemplateId,
      })
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 border-b border-soft bg-header-theme/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1900px] items-center justify-between gap-4 px-4 py-2.5 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div><p className="text-sm font-bold text-main">{meta.label}</p><p className="text-[10px] text-muted2">Editor de documento</p></div>
          </div>
      ...
      {step === "processing" && <div className="flex min-h-[75vh] flex-col items-center justify-center"><LoaderCircle size={42} className="animate-spin text-blue-600" /><p className="mt-4 text-sm font-bold text-main">Creando el documento...</p></div>}
