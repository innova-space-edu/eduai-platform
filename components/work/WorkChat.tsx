...
export function WorkChat({
  ...
  const sendMessage = useCallback(async (override?: string) => {
    ...
    try {
      ...
      // Los archivos se procesan como fuentes del Work. Las peticiones que se
      // refieren a ellas se responden con esos fragmentos, no con contenido
      // genérico; la autorización de este flujo se solicita al usuario al
      // adjuntar/usar una fuente.
      ...
      if (asksForSources && notebookId) {
        ...
        if (/\b(podcast|episodio|audio|narraci[oó]n)\b/i.test(content) && sourceData.text) {
          ...
          if (!audioResponse.ok || !audioData?.audioBase64) throw new Error(audioData?.error || "El guion se creó, pero no se pudo generar el audio")
  ...
  const attachSourceFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 20 * 1024 * 1024) {
      setMessages((current) => [...current, {
        id: messageId(), role: "assistant",
        content: "El archivo supera el límite de 20 MB. Reduce su tamaño o súbelo dividido para poder procesarlo correctamente.",
      }])
      return
    }
