...
export default function SourcePanel({ notebookId, sources, onSourcesChange }: SourcePanelProps) {
  ...
  // NUEVO: ingest robusto para resultados web
  ...
  // ─── Agregar UN resultado web (usa /api/web/ingest) ──────────────
    ...
    try {
      ...
      setMessage(
        duplicated
          ...
          : `Fuente agregada: ${result.title.slice(0, 60)}`
      )
  ...
  // ─── Agregar TODOS los resultados (usa /api/web/ingest) ──────────
  ...
  // ─── Procesar pendientes — SECUENCIAL ────────────────────────────

  const processPendingSources = async () => {
    if (!pendingSources.length) {
      setMessage("No hay fuentes pendientes.")
      return
    }
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Error procesando fuentes")
    } finally {
      setProcessing(false)
    }
  }
        ...
        {!addMode && (
          <>
            ...
            {pendingSources.length > 0 && (
              <button
                onClick={processPendingSources}
                disabled={processing || addingAll}
        ...
        <div className="p-4 border-b border-soft">
          ...
          {addMode === "file" && (
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={resetAdd}
                className="py-1.5 rounded-xl text-xs text-center"
                style={{ background: "var(--bg-card-soft)", color: "var(--text-muted)" }}
              >
                Cancelar
              </button>
