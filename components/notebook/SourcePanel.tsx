"use client"
// components/notebook/SourcePanel.tsx  v2
// Fix: safeJson en todos los fetch — nunca "Unexpected token '<'"
...
export default function SourcePanel({ notebookId, sources, onSourcesChange }: SourcePanelProps) {
  ...
  const resetAdd = () => {
    setAddMode(null); setUrlInput(""); setTextInput("")
    setTextTitle(""); setWebQuery(""); setWebResults([]); setError(null)
  }
  ...
  const ingestSource = async (sourceId: string, fileBase64?: string) => {
    try {
      ...
      console.error("[SourcePanel] ingest error:", e)
    }
  }
  ...
  const addUrl = async () => {
    ...
    try {
      ...
      const data = await safeJson<{ source: NotebookSource }>(res)
  ...
  const addText = async () => {
    ...
    try {
      const res  = await fetch(`/api/notebooks/${notebookId}/sources`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "text", title: textTitle.trim() || "Texto pegado", raw_text: textInput.trim() }),
      })
      const data = await safeJson<{ source: NotebookSource }>(res)
      await ingestSource(data.source.id)
      onSourcesChange(); resetAdd()
  ...
  const handleFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await safeJson<{ source: NotebookSource }>(res)
      await ingestSource(data.source.id, b64)
      onSourcesChange(); resetAdd()
    } catch (e: unknown) {
  ...
  const searchWeb = async () => {
    ...
    setWebLoading(true); setWebResults([])
    try {
      const res  = await fetch("/api/web/search", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query: webQuery.trim() }),
      })
      const data = await safeJson<{ results: WebSearchResult[] }>(res)
      setWebResults(data.results ?? [])
    } catch (e) {
      console.error("[SourcePanel] web search:", e)
      setWebResults([])
      ...
      {addMode && (
        <div className="p-4 border-b border-soft">
          ...
          {addMode === "web" && (
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <input type="text" placeholder="Buscar en la web..." value={webQuery}
                ...
                <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                  {webResults.map((r, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 rounded-xl text-xs"
                      ...
                      <button onClick={() => addWebResult(r)} disabled={loading}
              ...
              {webResults.length === 0 && !webLoading && webQuery && (
                <p className="text-xs text-muted2 text-center py-2">
                  Sin resultados. Configura SERPER_API_KEY para búsqueda completa.
                </p>
              )}
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
        {sources.length === 0 && (
          <div className="text-center py-10">
            ...
            <p className="text-xs text-muted2">Agrega fuentes para comenzar.<br />URL, texto, PDF o busca en la web.</p>
          </div>
        )}
        {sources.map((src) => (
...
function SourceCard({ source, onToggle, onDelete }: {
  source: NotebookSource; onToggle: () => void; onDelete: () => void
}) {
