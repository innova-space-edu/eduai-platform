...
if (!deepRoute.includes('.select("source_id,content_hash")') || !deepRoute.includes('.select("id,content_hash")') || !deepRoute.includes("indexedHashBySource")) {
  throw new Error("[google-file-search] content-hash freshness guard was not materialized")
}
...
if (changed) {
  ...
  console.log("[google-file-search] Notebook lifecycle + active/current Deep Research File Search integrados sin reemplazar RAG")
} else {
