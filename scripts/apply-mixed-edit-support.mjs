...
patch("app/examen/crear/page.tsx", [
  ...
  ['const [showResultToStudent, setShowResultToStudent] = useState(true);\n  const [allowReview, setAllowReview] = useState(true);', 'const [showResultToStudent, setShowResultToStudent] = useState(true);\n  const [allowReview, setAllowReview] = useState(true);'],
...
patch("app/api/agents/exam-generate/route.ts", [
  ...
  ['ESQUEMA OBLIGATORIO PARA true_false:', 'ESQUEMA OBLIGATORIO PARA mixed_choice_development:\n{\n  "type":"mixed_choice_development",\n  "question":"...",\n  "options":["...","...","...","..."],\n  "correctAnswer":0,\n  "answerText":"texto idéntico a options[correctAnswer]",\n  "explanation":"retroalimentación de la alternativa correcta",\n  "solutionSteps":["paso 1","paso 2"],\n  "distractorRationales":["motivo opción A","motivo opción B","motivo opción C","motivo opción D"],\n  "selectionPoints":3,\n  "developmentMaxPoints":2,\n  "modelAnswer":"desarrollo esperado",\n  "expectedLatex":"resultado final en LaTeX si corresponde",\n  "rubric":[{"criteria":"procedimiento correcto","points":1}]\n}\n\nESQUEMA OBLIGATORIO PARA true_false:'],
...
patch("app/examen/p/[code]/page.tsx", [
  ...
  ['const isDev = item.type === "development";\n                const isTF = item.type === "true_false";', 'const isDev = item.type === "development";\n                const isMixed = item.type === "mixed_choice_development";\n                const isTF = item.type === "true_false";'],
  ['const baseCorrect = g.isCorrect === true;', 'const baseCorrect = g.selectionCorrect === true || g.isCorrect === true;'],
  ['const state = isDev\n                  ? "dev"', 'const mixedSelPts = Number(g.selectionPoints ?? item.selectionPoints ?? 0) || 0;\n                const mixedDevScore = Math.max(0, Number(g.developmentScore ?? g.aiScore ?? 0) || 0);\n                const mixedDevMax = Math.max(0, Number(g.developmentMaxPoints ?? item.developmentMaxPoints ?? 0) || 0);\n                const mixedEarned = (baseCorrect ? mixedSelPts : 0) + mixedDevScore;\n                const mixedTotal = mixedSelPts + mixedDevMax;\n                const mixedFull = isMixed && mixedTotal > 0 && mixedEarned >= mixedTotal;\n                const mixedPartial = isMixed && mixedEarned > 0 && mixedEarned < mixedTotal;\n                const state = isDev\n                  ? "dev"\n                  : isMixed\n                    ? mixedFull\n                      ? "full"\n                      : mixedPartial\n                        ? "partial"\n                        : "wrong"'],
  ['const studentAnswer = isDev\n                  ? g.devText || "—"\n                  : isTF', 'const studentAnswer = isDev\n                  ? g.devText || "—"\n                  : isMixed\n                    ? `Alternativa: ${item.options?.[g.selectedAnswer] || "—"}\\nDesarrollo: ${g.developmentLatex || g.devText || "—"}`\n                    : isTF'],
  ['const correctAnswer = isDev\n                  ? item.modelAnswer ||\n                    item.expectedLatex ||\n                    item.expectedAnswer ||\n                    "Ver rúbrica"\n                  : item.answerText ||', 'const correctAnswer = isDev\n                  ? item.modelAnswer ||\n                    item.expectedLatex ||\n                    item.expectedAnswer ||\n                    "Ver rúbrica"\n                  : isMixed\n                    ? `Alternativa: ${item.options?.[item.correctAnswer] || item.answerText || "—"}\\nDesarrollo modelo: ${item.modelAnswer || item.expectedLatex || "Ver rúbrica"}`\n                    : item.answerText ||'],
]);
