...
def parse_subject(url: str, subject: dict, course: int) -> list[dict]:
    soup = BeautifulSoup(fetch_html(url), "html.parser")
    axes_lookup = {clean(axis).casefold(): axis for axis in subject["axes"]}
    current_axis: str | None = None
    collected: list[dict] = []
    code_pattern = re.compile(rf"\b{subject['prefix']}{course:02d} OA (\d{{2}})\b")
    ...
    for heading in soup.find_all(["h3", "h4"]):
        ...
        code = f"{subject['prefix']}{course:02d} OA {number:02d}"
        collected.append(
            {
                "number": number,
                "code": code,
    ...
    expected_codes = [f"{subject['prefix']}{course:02d} OA {number:02d}" for number in range(1, subject["count"] + 1)]
    actual_codes = [item["code"] for item in result]
    if actual_codes != expected_codes:
        raise RuntimeError(
            f"{subject['name']}: secuencia oficial inesperada.\nEsperada: {expected_codes}\nObtenida: {actual_codes}"
        )
    return result


def build_file(course: int, course_cfg: dict, subject: dict, url: str, objectives: list[dict]) -> dict:
    units = []
    for index, axis in enumerate(subject["axes"], start=1):
        axis_objectives = [item for item in objectives if item["axis"] == axis]
        if not axis_objectives:
            raise RuntimeError(f"{subject['name']}: eje sin OA extraídos: {axis}")
        units.append(
            {
                "numero": index,
                ...
                "oa": [
                    {
                        "id": f"{subject['prefix']}{course}B-OA-{item['number']:02d}",
...
def update_registry(course: int, course_cfg: dict, results: list[tuple[dict, str]]) -> None:
    ...
    data["revision"] = f"fase_8_{course}_basico_completo_verificado"
    data["fecha_actualizacion"] = CONSULT_DATE
    registry = data.setdefault("asignaturas_verificadas", {})

    for subject, url in results:
        key = f"basica/{course_cfg['folder']}/{subject['file'].removesuffix('.json')}"
        registry[key] = {
            "url": url,
            "base_curricular_id": BASE_CURRICULAR_ID,
            "fecha_consulta": CONSULT_DATE,
            "alcance": [
                f"{subject['prefix']}{course:02d} OA 01",
                f"{subject['prefix']}{course:02d} OA {subject['count']:02d}",
            ],
            "cantidad_oa_contenido": subject["count"],
        }

    path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def update_progress(course: int, course_cfg: dict) -> None:
    path = MINEDUC_ROOT / "meta" / "verification-progress.json"
    data = json.loads(path.read_text(encoding="utf-8"))
    data.update(
        {
            "fecha_actualizacion": CONSULT_DATE,
            "fase": f"{course}_basico_completo",
            ...
            "nota": "Contabiliza OA de contenido cotejados con páginas oficiales específicas. Inglés de 1° a 4° básico se registra como propuesta oficial; Inglés de 5° básico pertenece a la base curricular obligatoria.",
        }
    )
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    for subject in course_cfg["subjects"]:
        url = f"https://www.curriculumnacional.cl/curriculum/1o-6o-basico/{subject['slug']}/{course}-basico"
        objectives = parse_subject(url, subject, course)
        total += len(objectives)
        output = build_file(course, course_cfg, subject, url, objectives)
        destination = output_dir / subject["file"]
        destination.write_text(json.dumps(output, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        written.append((subject, url))
        print(f"[ok] {subject['name']}: {len(objectives)} OA -> {destination.relative_to(ROOT)}")
    ...
    update_progress(course, course_cfg)
