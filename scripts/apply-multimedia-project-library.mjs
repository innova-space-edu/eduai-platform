  ...
  `  function removeAsset(asset: StudioAsset) {
    ...
    if (usedClips.length) {
      const accepted = window.confirm(
        `El archivo “${asset.name}” está usado en ${usedClips.length} clip(s). ¿Eliminar también esos clips de la línea de tiempo?`,
      );
      if (!accepted) return;
...
replaceOnce(
  ...
  `            <select value={exportFormat} onChange={(event) => setExportFormat(event.target.value as StudioExportFormat)} className="rounded-xl border border-white/10 bg-[#0b1020] px-2 py-2 text-xs" aria-label="Formato de exportación">
            ...
            <button disabled={exporting} onClick={() => void exportCurrentFormat()} className="rounded-xl bg-cyan-600 px-3 py-2 text-xs font-semibold hover:bg-cyan-500 disabled:opacity-50"><Download size={14} className="mr-1 inline" />{exporting ? (exportFormat === "mp3" || exportFormat === "wav" ? "Exportando…" : \`${Math.round(exportProgress * 100)}%\`) : "Exportar"}</button>`,
  "selector superior MP3/WAV",
);
