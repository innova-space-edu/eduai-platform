...
begin
  ...
  if new.birth_date > (current_date - interval '18 years')::date then
    ...
    new.age_band = 'adult';
    -- Los roles privilegiados (teacher/researcher/admin) solo los eleva el backend/admin.
    if tg_op = 'INSERT' or old.access_tier not in ('teacher','researcher','admin') then
