  ...
  try {
    ...
    if (action === "delete") {
      ...
      if (exam.status === "active") {
        return NextResponse.json({ error: "Cierra el examen antes de enviarlo a la papelera" }, { status: 400 })
      }

      // Soft delete: solo marcar deleted_at, no borrar datos
    ...
    if (action === "permanent_delete") {
      ...
      // Verificar que el examen está en papelera (deleted_at no null)
      const { data: exam } = await supabase
        .from("teacher_exams")
      ...
      if (!exam.deleted_at) {
        return NextResponse.json({ error: "El examen debe estar en la papelera antes de eliminarse permanentemente" }, { status: 400 })
      }

      await supabase.from("exam_submissions").delete().eq("exam_id", examId)
      const { error: delErr } = await supabase
        .from("teacher_exams").delete().eq("id", examId).eq("teacher_id", teacherId)
