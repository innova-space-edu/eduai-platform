    ...
    <span>
      {parts.map((part, i) => {
        ...
        if (isBlock || isInline) {
          ...
          const html = tex
            .replace(/\\frac\{([^}]+)\}\{([^}]+)\}/g, "<span class='inline-flex flex-col items-center mx-1'><span class='border-b border-blue-400/50 px-1'>$1</span><span class='px-1'>$2</span></span>")
            .replace(/\\sqrt\{([^}]+)\}/g, "√($1)")
            .replace(/\\cdot/g, "·").replace(/\\times/g, "×").replace(/\\div/g, "÷")
            .replace(/\\leq/g, "≤").replace(/\\geq/g, "≥").replace(/\\neq/g, "≠").replace(/\\pm/g, "±")
            .replace(/\\pi/g, "π").replace(/\\alpha/g, "α").replace(/\\beta/g, "β").replace(/\\theta/g, "θ")
...
export default function ExamenPublicoPage() {
  ...
  useEffect(() => {
    if (!code) return
    fetch(`/api/agents/examen-docente?code=${code}`).then(r => r.json()).then(d => {
      if (d.error) { setErrorMsg(d.error); setPhase("error"); return }
      setExam(d.exam); setTimeLeft((d.exam.settings?.timeLimit || 30) * 60); setPhase("register")
    }).catch(() => { setErrorMsg("Error cargando examen"); setPhase("error") })
  }, [code])

  useEffect(() => {
    if (phase !== "exam") return
    timerRef.current = setInterval(() => setTimeLeft(p => { if (p <= 1) { doSubmit(); return 0 } return p - 1 }), 1000)
    return () => { if (timerRef.current) clearInterval(timerRef.current) }
  }, [phase])

  const startExam = () => { if (!name.trim() || !course.trim()) return; startRef.current = Date.now(); setPhase("exam") }
  ...
  const doSubmit = async () => {
    ...
    const ansArr = qs.map((q: any, i: number) => {
      if (q.type === "development") return { devText: devAnswers[i] || "", selectedAnswer: -1 }
      if (q.type === "true_false") return { selectedAnswer: mcAnswers[i] ?? -1, justification: tfJustifications[i] || "" }
      return { selectedAnswer: mcAnswers[i] ?? -1 }
    })
    try {
      const res = await fetch("/api/agents/examen-docente", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "submit", examId: exam.id, studentName: name, studentCourse: course,
          studentRut: rut || null, answers: ansArr, questions: qs,
      ...
      setSubmission(d.submission); setPhase("review")
  ...
  if (phase === "error") return <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4"><div className="text-center"><div className="text-4xl mb-3">❌</div><h2 className="text-white font-bold text-lg mb-2">Examen no disponible</h2><p className="text-gray-500 text-sm">{errorMsg}</p></div></div>
  if (phase === "submitting") return <div className="min-h-screen bg-gray-950 flex items-center justify-center"><div className="text-center"><div className="w-12 h-12 rounded-full border-2 border-white/10 border-t-green-400 animate-spin mx-auto mb-3" /><p className="text-gray-400 text-sm">Evaluando respuestas con IA...</p><p className="text-gray-600 text-xs mt-1">Esto puede tomar unos segundos</p></div></div>
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full space-y-6">
        <div className="text-center">
          <div className="w-16 h-16 rounded-2xl bg-blue-600/20 border border-blue-500/30 flex items-center justify-center mx-auto mb-3"><span className="text-3xl">📝</span></div>
        ...
        <div className="space-y-3">
          <div><label className="text-gray-400 text-xs font-semibold block mb-1">NOMBRE COMPLETO *</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Juan Pérez López" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-2xl px-4 py-3 text-gray-200 text-sm focus:outline-none focus:border-blue-500/30" /></div>
          <div><label className="text-gray-400 text-xs font-semibold block mb-1">CURSO *</label>
            <input value={course} onChange={e => setCourse(e.target.value)} placeholder="8°A, 1° Medio B" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-2xl px-4 py-3 text-gray-200 text-sm focus:outline-none focus:border-blue-500/30" /></div>
          <div><label className="text-gray-400 text-xs font-semibold block mb-1">RUT (opcional)</label>
            <input value={rut} onChange={e => setRut(e.target.value)} placeholder="12.345.678-9" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-2xl px-4 py-3 text-gray-200 text-sm focus:outline-none focus:border-blue-500/30" /></div>
        </div>
        <button onClick={startExam} disabled={!name.trim() || !course.trim()} className="w-full py-3.5 rounded-2xl bg-blue-600/90 hover:bg-blue-500 text-white font-bold text-sm disabled:opacity-30">Comenzar examen →</button>
      </div>
    </div>
  )
      ...
      <div className="min-h-screen bg-gray-950 px-4 py-6">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-6">
            <div className="text-5xl mb-2">{nota >= 5.5 ? "🎉" : nota >= 4.0 ? "📚" : "💪"}</div>
            {showRes ? (<>
              ...
              <div className="w-full bg-gray-800 rounded-full h-3 mt-4"><div className={`h-full rounded-full ${nota >= 4.0 ? "bg-green-500" : "bg-red-500"}`} style={{ width: `${pct}%` }} /></div>
            </>) : (<><h2 className="text-xl font-bold text-white">Examen enviado</h2><p className="text-gray-500 text-sm mt-1">Tu docente revisará tus respuestas</p></>)}
          </div>

          {showRes && (
            <div className="space-y-3">
              <h3 className="text-gray-400 text-xs font-semibold tracking-widest">REVISIÓN DETALLADA</h3>
              {qs.map((q: any, i: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    {q.type === "multiple_choice" && (
                      <div className="space-y-1 mb-3">
                        {(q.options || []).map((opt: string, j: number) => {
                          ...
                          return (
                            <div key={j} className={`text-xs px-3 py-1.5 rounded-lg ${isRight ? "bg-green-500/10 text-green-400 border border-green-500/20" : isStudent ? "bg-red-500/10 text-red-400 border border-red-500/20 line-through" : "text-gray-600"}`}>
                              {String.fromCharCode(65 + j)}) <MathText text={opt} /> {isRight && "✓"} {isStudent && !isRight && "← tu respuesta"}
                            </div>
                          )
                        })}
                      </div>
                    ...
                    {/* TF review */}
                    {isTF && (<div className="space-y-2 mb-3">
                      <div className="flex gap-2">
                        {(q.options || []).map((opt: string, j: number) => (
                          <div key={j} className={`text-xs px-3 py-1.5 rounded-lg flex-1 text-center ${j === q.correctAnswer ? "bg-green-500/10 text-green-400 border border-green-500/20" : g.selectedAnswer === j ? "bg-red-500/10 text-red-400 border border-red-500/20" : "text-gray-600"}`}>
                            {opt} {j === q.correctAnswer && "✓"} {g.selectedAnswer === j && j !== q.correctAnswer && "✗"}
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Dev review */}
                    {isDev && (<div className="space-y-2 mb-3">
                      <div className="bg-white/[0.03] rounded-lg p-2.5 border border-white/[0.06]"><p className="text-gray-600 text-[10px] font-semibold">TU RESPUESTA:</p><p className="text-gray-300 text-xs">{g.devText || "Sin respuesta"}</p></div>
                      <div className="bg-green-500/[0.05] rounded-lg p-2.5 border border-green-500/10"><p className="text-green-400 text-[10px] font-semibold">RESPUESTA MODELO:</p><p className="text-gray-300 text-xs"><MathText text={q.modelAnswer || ""} /></p></div>
                      {g.aiFeedback && (<div className="bg-blue-500/[0.05] rounded-lg p-2.5 border-l-2 border-blue-500/30"><p className="text-blue-400 text-[10px] font-semibold">EVALUACIÓN IA ({g.aiScore}/{g.aiMaxScore} pts):</p><p className="text-gray-400 text-xs">{g.aiFeedback}</p></div>)}
                    </div>)}

                    {q.explanation && !isDev && (<div className="bg-blue-500/[0.05] rounded-lg p-2.5 border-l-2 border-blue-500/30"><p className="text-blue-400 text-[10px] font-semibold">EXPLICACIÓN:</p><p className="text-gray-400 text-xs"><MathText text={q.explanation} /></p></div>)}
                  </div>
                )
              })}
            </div>
          )}
          <div className="text-center mt-6"><p className="text-gray-600 text-xs">{exam?.title} — {name} ({course})</p><p className="text-gray-700 text-xs mt-1">Puedes cerrar esta ventana</p></div>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="sticky top-0 z-10 bg-gray-950/90 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-2xl mx-auto px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-3"><span className="text-gray-400 text-xs">{name}</span><span className="text-gray-700">|</span><span className="text-gray-500 text-xs">{answeredCount}/{totalQ}</span></div>
          <div className={`font-mono font-bold text-sm ${timeLeft < 60 ? "text-red-400 animate-pulse" : timeLeft < 300 ? "text-yellow-400" : "text-gray-400"}`}>⏱ {fmt(timeLeft)}</div>
        </div>
        <div className="h-1 bg-gray-800"><div className="h-full bg-blue-500 transition-all" style={{ width: `${(answeredCount / totalQ) * 100}%` }} /></div>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-blue-400 text-xs font-semibold">Pregunta {curQ + 1} de {totalQ}</span>
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${q.type === "multiple_choice" ? "bg-blue-500/10 text-blue-400" : q.type === "true_false" ? "bg-green-500/10 text-green-400" : "bg-orange-500/10 text-orange-400"}`}>
                  {q.type === "multiple_choice" ? "Alternativas" : q.type === "true_false" ? "V/F + Justificación" : "Desarrollo"}
                </span>
              </div>
            ...
            <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5">
              <p className="text-white font-semibold text-sm leading-relaxed"><MathText text={q.question} /></p>
            </div>
            ...
            {q.type === "multiple_choice" && (
              <div className="space-y-2">
                {(q.options || []).map((opt: string, i: number) => (
                  <button key={i} onClick={() => setMcAnswers({ ...mcAnswers, [curQ]: i })}
                    className={`w-full text-left p-3.5 rounded-xl border text-sm transition-all ${mcAnswers[curQ] === i ? "bg-blue-500/10 border-blue-500/30 text-blue-300" : "bg-white/[0.03] border-white/[0.06] text-gray-400 hover:bg-white/[0.06]"}`}>
                    <span className="font-bold mr-2">{String.fromCharCode(65 + i)}.</span><MathText text={opt} />
                  </button>
                ))}
              </div>
            )}

            {/* TF options + justification */}
            {q.type === "true_false" && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  {(q.options || ["Verdadero", "Falso"]).map((opt: string, i: number) => (
                    <button key={i} onClick={() => setMcAnswers({ ...mcAnswers, [curQ]: i })}
                      className={`p-3.5 rounded-xl border text-sm font-semibold text-center transition-all ${mcAnswers[curQ] === i ? "bg-blue-500/10 border-blue-500/30 text-blue-300" : "bg-white/[0.03] border-white/[0.06] text-gray-400 hover:bg-white/[0.06]"}`}>
                      {opt}
                    </button>
                  ))}
                </div>
                <div>
                  <label className="text-gray-500 text-xs font-semibold block mb-1">JUSTIFICA TU RESPUESTA *</label>
            ...
            {q.type === "development" && (
              <div>
                <textarea value={devAnswers[curQ] || ""} onChange={e => setDevAnswers({ ...devAnswers, [curQ]: e.target.value })}
                ...
                <p className="text-gray-600 text-[10px] mt-1">Puntaje máximo: {q.maxPoints || 5} puntos • Evaluado por IA</p>
              </div>
            )}

            {/* Nav */}
            <div className="flex gap-3 pt-2">
              {curQ > 0 && <button onClick={() => setCurQ(curQ - 1)} className="px-4 py-2.5 rounded-xl border border-white/10 text-gray-500 text-sm">← Anterior</button>}
              <div className="flex-1" />
              {curQ < totalQ - 1 ? (
                <button onClick={() => setCurQ(curQ + 1)} className="px-4 py-2.5 rounded-xl bg-blue-600/80 text-white text-sm font-semibold">Siguiente →</button>
              ) : (
                <button onClick={doSubmit} className="px-6 py-2.5 rounded-xl bg-green-600/90 text-white text-sm font-bold">✅ Enviar examen</button>
              )}
            </div>
