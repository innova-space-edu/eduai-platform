...
export default function Dashboard() {
        ...
        .then(({ data }) => {
          if (data) {
            ...
            const lvls = [
              ...
              { name: "Practicante",  min: 500 },
              { name: "Avanzado",     min: 1200 },
              { name: "Experto",      min: 2500 },
              { name: "Maestro",      min: 5000 },
            ]
            const lvl = [...lvls].reverse().find(l => (data.xp || 0) >= l.min)
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      ...
      {/* Sidebar */}
      <aside style={{ width: sw }} className="fixed left-0 top-0 h-full bg-gray-900 border-r border-gray-800 flex flex-col z-20 transition-all duration-300 overflow-hidden">
        ...
        <div className="flex-1 py-3 flex flex-col overflow-y-auto overflow-x-hidden">
          {expanded && <p className="text-gray-600 text-[10px] uppercase tracking-widest px-4 mb-2">Agentes</p>}
          {AGENTS.map(a => (
            <Link key={a.id} href={a.href} title={!expanded ? a.name : undefined}
              ...
              {expanded && <span className="text-gray-400 group-hover:text-white text-sm font-medium whitespace-nowrap">{a.name}</span>}
            </Link>
          ))}
        </div>
        ...
        <div className="border-t border-gray-800 py-3 flex flex-col flex-shrink-0">
          {BOTTOM_LINKS.map(item => (
            <Link key={item.href} href={item.href} title={!expanded ? item.label : undefined}
              ...
              {expanded && <span className="text-gray-500 group-hover:text-gray-200 text-sm whitespace-nowrap">{item.label}</span>}
            </Link>
          ))}
        </div>
      </aside>
        ...
        <div className="max-w-2xl mx-auto w-full px-6 py-8 flex flex-col gap-6">
          ...
          {ready && (
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 text-center">
              <p className="text-green-400 text-sm">✓ Datos cargados correctamente</p>
            </div>
          )}
        </div>
      </main>
