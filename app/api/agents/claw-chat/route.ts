...
function buildSuggestions(reply: string, toolUsed?: string) {
  ...
  for (const agent of Object.values(AGENT_ROUTES)) {
    if (
      reply.includes(agent.href) &&
      !suggestions.find((s) => s.href === agent.href)
    ) {
  ...
  if (key) {
    ...
    if (agent && !suggestions.find((s) => s.href === agent.href))
      suggestions.unshift(agent);
  }
