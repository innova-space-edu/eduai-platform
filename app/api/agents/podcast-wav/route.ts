...
function normalizeSegments(raw: any): Segment[] {
  ...
  return segs
    ...
    .map((s) => ({ speaker: s.speaker === "B" ? "B" : "A", text: String(s.text || "").trim() }))
...
function splitText(text: string, maxLen = 900): string[] {
  // Cortes suaves por frases para robustez.
  const t = (text || "").trim();
  if (!t) return [];
...
function extractWavPCM(wav: ArrayBuffer): { pcm: Uint8Array; sampleRate: number; numChannels: number; bitsPerSample: number } {
  ...
  for (let i = 0; i < wav.byteLength - 24; i++) {
    if (view.getUint8(i) === 0x66 && view.getUint8(i + 1) === 0x6d && view.getUint8(i + 2) === 0x74 && view.getUint8(i + 3) === 0x20) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (segments.length === 0) return NextResponse.json({ error: "No hay segmentos" }, { status: 400 });
    ...
    const tts = new EdgeTTS({ outputFormat: OUTPUT_FORMAT.RIFF_24KHZ_16BIT_MONO_PCM });
    ...
    for (const seg of grouped) {
      ...
      for (const chunk of chunks) {
        const audio = await tts.synthesize(chunk, voice, {
          // Ajustes suaves tipo podcast
          rate: seg.speaker === "B" ? 5 : 0,
          pitch: seg.speaker === "B" ? 2 : -2,
          volume: 100,
        });
    ...
    if (pcmParts.length === 0) return NextResponse.json({ error: "No se generó audio" }, { status: 502 });
