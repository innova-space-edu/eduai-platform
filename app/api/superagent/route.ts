...
import { runSuperAgentCycle } from "@/lib/superagent/engine"
...
export async function GET() {
  return Response.json({
    ...
    features: SUPERAGENT_CONFIG.features,
    limits: SUPERAGENT_CONFIG.limits,
    message: "EduAI Claw está disponible.",
  })
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await runSuperAgentCycle(context)
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No se pudo procesar la solicitud del superagente."
