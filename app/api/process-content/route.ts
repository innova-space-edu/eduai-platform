  ...
  try {
    ...
    if (!sourceType || !content || !outputFormat) {
      return NextResponse.json(
        { success: false, error: "Faltan campos: sourceType, content, outputFormat" },
        { status: 400 }
      )
    }
    ...
    if (!VALID_SOURCES.includes(sourceType)) {
      return NextResponse.json(
        { success: false, error: `sourceType inválido. Válidos: ${VALID_SOURCES.join(", ")}` },
        { status: 400 }
      )
    }
    ...
    if (!VALID_FORMATS.includes(outputFormat)) {
      return NextResponse.json(
        { success: false, error: `outputFormat inválido. Válidos: ${VALID_FORMATS.join(", ")}` },
        { status: 400 }
      )
    }
    ...
    const result = await processContent({ sourceType, content, fileName, outputFormat, geminiKey })
    ...
    return NextResponse.json(
      { success: false, error: `Error interno: ${err.message}` },
      { status: 500 }
    )
  }
}
