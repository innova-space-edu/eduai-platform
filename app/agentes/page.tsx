...
const AGENTS: AgentItem[] = [
  ...
  },
  {
    ...
    description:
      "Cuaderno digital con dibujo libre, figuras y gráficos 2D/3D, cámara e imágenes o stickers con IA.",
  ...
  {
    id: "cuaderno-creativo",
    icon: "🖍️",
    name: "Cuaderno creativo",
    description:
      "Dibuja, pinta y crea cuadernos con plantillas para colorear subidas o generadas con IA.",
...
const TAG_STYLES: Record<string, { bg: string; text: string; border: string }> = {
  ...
  "Cuaderno digital": { bg: "rgba(14,165,233,0.1)", text: "#7dd3fc", border: "rgba(14,165,233,0.2)" },
    ...
    <div className="min-h-screen bg-app">
      <header className="sticky top-0 z-10 border-b border-soft bg-app backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3.5">
          <Link href="/dashboard" aria-label="Volver al inicio" className="flex h-8 w-8 items-center justify-center rounded-xl border border-soft bg-card-soft-theme text-sub transition-all hover:bg-input-theme hover:text-main">
            <ArrowLeft size={15} />
          </Link>
          <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 shadow-md shadow-blue-500/20">🤖</div>
          <div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        <div className="mb-8 flex flex-wrap gap-2">
          {TAGS.map((tag) => {
            ...
            const count = AGENTS.filter((agent) => agent.tag === tag).length;
            return (
              <div key={tag} className="flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium" style={{ background: style.bg, color: style.text, borderColor: style.border }}>
                <span>{tag}</span>
