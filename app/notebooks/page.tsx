"use client"
// app/notebooks/page.tsx  v3 — EduAI Notebooks
...
export default function NotebooksPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-app">
      {/* Topbar */}
      <div className="border-b border-soft sticky top-0 z-10 backdrop-blur-xl"
        style={{ background: "var(--bg-header)" }}>
        <div className="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-lg">📓</span>
            <div>
              <p className="text-main font-bold text-sm leading-tight">EduAI Notebooks</p>
              <p className="text-[10px] text-muted2 leading-tight">Investigación desde fuentes reales</p>
            </div>
          </div>
      ...
      <div className="max-w-3xl mx-auto w-full px-6 py-10 flex flex-col gap-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted2 text-sm leading-relaxed max-w-lg">
              Agrega fuentes reales (URLs, PDFs, texto), conversa con un especialista
              y genera materiales educativos desde el contenido verificado.
            </p>
          </div>
          {!showCreate && (
            <button onClick={() => setShowCreate(true)}
              className="flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm font-semibold text-white flex-shrink-0"
        ...
        {showCreate && (
          <div className="rounded-2xl border p-5 flex flex-col gap-3"
            ...
            <input autoFocus type="text" placeholder="Nombre del cuaderno..."
        ...
        {!loading && notebooks.length === 0 && !showCreate && (
          <div className="text-center py-16">
            ...
            <p className="text-muted2 text-sm mb-6">Crea tu primer cuaderno y empieza a investigar con IA.</p>
            <button onClick={() => setShowCreate(true)}
        ...
        {!loading && notebooks.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {notebooks.map((nb) => (
              <button key={nb.id} onClick={() => router.push(`/notebooks/${nb.id}`)}
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-main leading-tight truncate">{nb.title}</p>
                      <p className="text-[11px] text-muted2 truncate mt-0.5">{nb.specialist_role}</p>
                    </div>
                  </div>
                  <button onClick={(e) => deleteNotebook(nb.id, e)}
        ...
        <div className="rounded-2xl px-5 py-4 border text-sm text-muted2 leading-relaxed"
          ...
          <span className="text-blue-400 font-semibold">💡 Flujo: </span>
          Agrega fuentes verificadas → procésalas → conversa con el especialista →
          genera <strong className="text-sub">infografías</strong>, <strong className="text-sub">podcasts con audio</strong>,{" "}
