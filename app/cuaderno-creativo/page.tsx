...
async function prepareTemplate(source: string) {
  ...
  if (!context) {
    throw new Error("El navegador no permite preparar la plantilla.");
  }
...
export default function CuadernoCreativoPage() {
  ...
  const [notebook, setNotebook] = useState<CreativeNotebook>(() =>
    createNotebook(),
  );
  ...
  function clearDrawing() {
    ...
    const context = canvas?.getContext("2d");
    if (!canvas || !context) return;
    if (!window.confirm("¿Borrar todos los trazos y colores de esta página?")) {
      return;
    }
    context.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    commitDrawing();
  ...
  function floodFill(point: Point) {
    ...
    const composed = composedContext.getImageData(
      0,
      0,
      CANVAS_WIDTH,
      CANVAS_HEIGHT,
    );
    const foreground = drawingContext.getImageData(
      0,
      0,
      CANVAS_WIDTH,
      CANVAS_HEIGHT,
  ...
  function onPointerDown(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (tool === "line" || tool === "rectangle" || tool === "ellipse") {
      previewImageRef.current = context.getImageData(
        0,
        0,
        CANVAS_WIDTH,
        CANVAS_HEIGHT,
  ...
  function onPointerMove(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (tool === "line" || tool === "rectangle" || tool === "ellipse") {
      if (previewImageRef.current) {
        context.putImageData(previewImageRef.current, 0, 0);
      }
      ...
      if (!response.ok) {
        throw new Error(
          typeof data.error === "string"
            ? data.error
            : raw.trim() || "No fue posible generar la plantilla.",
        );
      }
  ...
  async function downloadCurrentPage() {
    ...
    try {
      ...
      link.download = `${sanitizeFileName(notebook.title)}-${sanitizeFileName(
        activePage.title,
      )}.png`;
      ...
      setSaveStatus(
        error instanceof Error
          ? error.message
          : "No fue posible descargar la página",
      );
    } finally {
      setExporting(false);
    }
  ...
  async function downloadNotebookPdf() {
    ...
    try {
      ...
      for (let index = 0; index < notebook.pages.length; index += 1) {
        if (index > 0) {
          pdf.addPage([CANVAS_WIDTH, CANVAS_HEIGHT], "portrait");
        }
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Dibuja, pinta y crea plantillas para colorear con IA
              </p>
            </div>
          </div>
      ...
      <main className="mx-auto grid max-w-[1600px] gap-4 p-4 lg:grid-cols-[270px_minmax(0,1fr)]">
        <aside className="self-start overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:sticky lg:top-[78px] lg:max-h-[calc(100vh-94px)] lg:overflow-y-auto">
          ...
          <div className="border-b border-slate-200 p-4">
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 py-2.5 text-xs font-bold text-white transition hover:bg-blue-500"
            >
            ...
            {activePage.backgroundImage && (
              <label className="mt-3 block text-xs font-semibold text-slate-600">
                Intensidad de plantilla: {Math.round(activePage.templateOpacity * 100)}%
                <input
                  type="range"
                  min="15"
                  max="100"
          ...
          <div className="p-4">
            ...
            <p className="mb-3 text-xs leading-relaxed text-slate-500">
              Describe la lámina. La IA creará una plantilla en blanco y negro lista para colorear.
            </p>
            <textarea
              value={aiPrompt}
            ...
            >
              <Sparkles size={15} />
              {generating ? "Generando..." : "Crear plantilla"}
            </button>
          </div>
        </aside>
        ...
        <section className="min-w-0 rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 px-4 py-3 text-xs text-slate-500">
            <span>
              <strong className="text-slate-700">{activePage.title}</strong> · Herramienta: {TOOLS.find((item) => item.id === tool)?.label}
            </span>
