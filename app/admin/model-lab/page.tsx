    ...
    <main className={`${styles.darkLab} min-h-screen px-3 py-5 text-white sm:px-5 sm:py-7`}>
      ...
      <div className="mx-auto max-w-[1440px] space-y-5">
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-5">
            <div className="max-w-4xl">
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-2 rounded-full border border-amber-400/20 bg-amber-950/30 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-amber-200">
            ...
            <Link
              href="/admin"
              className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-[#0c1a2d] px-4 py-2.5 text-sm font-black text-slate-200 transition hover:-translate-y-0.5 hover:bg-[#10223a]"
            >
        ...
        <div id="experimental" className="scroll-mt-24 space-y-4">
          <section className="overflow-hidden rounded-[28px] border border-amber-400/20 bg-[#111723] p-5 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <span className="inline-flex items-center gap-2 rounded-full border border-red-400/20 bg-red-950/30 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-red-200">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {ADMIN_ONLY_EXPERIMENTAL_MODELS.map((model) => (
              <article key={model.id} className="rounded-[28px] border border-white/10 bg-[#08111f] p-5 transition hover:border-white/15 sm:p-6">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full border border-red-400/20 bg-red-950/30 px-3 py-1 text-xs font-black text-red-200">
                    {model.enabledByDefault ? "Activo" : "Apagado"}
                  </span>
                </div>

                <div className="mt-5 grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
                  <div className="rounded-2xl border border-emerald-400/15 bg-emerald-950/25 p-3">
                    <p className="text-xs font-black text-emerald-200">Permitido</p>
                    <ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] leading-5 text-emerald-100/75">
                      {model.allowedContexts.map((item) => <li key={item}>{item}</li>)}
                    </ul>
                  </div>
                  <div className="rounded-2xl border border-red-400/15 bg-red-950/25 p-3">
                    <p className="text-xs font-black text-red-200">Bloqueado</p>
                    <ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] leading-5 text-red-100/75">
                      {model.blockedContexts.map((item) => <li key={item}>{item}</li>)}
                    </ul>
                  </div>
                  <div className="rounded-2xl border border-blue-400/15 bg-blue-950/25 p-3">
                    <p className="text-xs font-black text-blue-200">Controles</p>
                    <ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] leading-5 text-blue-100/75">
