...
def root() -> dict[str, Any]:
    return {"ok": True, "service": APP_NAME, "endpoints": ["/health", "/pipeline"]}
...
def run_pipeline(payload: PipelineRequest, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    suffix = _safe_suffix(payload.fileName, payload.mimeType)
    with tempfile.TemporaryDirectory(prefix="eduai-audio-") as temp_dir:
        audio_path = Path(temp_dir) / f"input{suffix}"
        audio_path.write_bytes(audio)

        model = _get_model()
        word_timestamps = payload.options.mode == "pro" or payload.options.preciseSubtitles
        segments_gen, info = model.transcribe(
            str(audio_path),
            beam_size=5 if payload.options.mode == "pro" else 3,
            vad_filter=True,
