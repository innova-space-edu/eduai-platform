...
function getForwardedHeader(headers: HeadersInit | undefined, name: string): string | null {
...
async function callInternalAPI(
  ...
  const safeBody = path === "/api/superagent/chat"
    ? { ...body, skipTools: true }
    : body
...
function pickSubject(message: string): string {
  ...
  if (/matem[aá]tica|ecuaci[oó]n|funci[oó]n|geometr/i.test(m)) return "Matemática"
  ...
  if (/biolog[ií]a|c[eé]lula|ecosistema|gen[eé]tica/i.test(m)) return "Biología"
...
export const TOOL_REGISTRY: ToolDefinition[] = [
  {
    ...
    params: [
      ...
      { name: "count",    type: "number", description: "Cantidad de preguntas",                 required: false },
...
Para development: omite options y correctAnswer, agrega "modelAnswer": "respuesta esperada".`
...

  {
    ...
    icon:        "♿",
    description: "Adapta texto, preguntas o instrucciones para estudiantes con NEE.",
    ...
    async execute(args, baseUrl, options) {
      try {
        ...
        ].filter(Boolean).join("; ") || "necesidades educativas generales"
...
Devuelve SOLO el texto adaptado.`
        ...
        return { success: true, output: `♿ **Texto adaptado para PIE** (${needs}):\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...

  {
    ...
    params: [
      { name: "subject",  type: "string", description: "Asignatura",   required: false },
      { name: "level",    type: "string", description: "Nivel/Curso",  required: false },
    ...
    async execute(args, baseUrl, options) {
      try {
        const topic = normalizeText(args.topic)
        const subject = normalizeText(args.subject) || pickSubject(topic) || "Asignatura por definir"
...
Incluye para cada clase: OA sugerido, indicadores, inicio, desarrollo, cierre, recursos y evaluación formativa.`
        ...
        return { success: true, output: `📚 **Planificación generada** — ${subject} · ${level}:\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...

  {
    ...
    async execute(args, baseUrl, options) {
      try {
        ...
        const level = normalizeText(args.level) || "estudiante de enseñanza media"
...
5. Pregunta de reflexión.`
        ...
        return { success: true, output: `🔬 **Explicación de "${concept}"**:\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...

  {
    ...
    enabled:     true,
    params: [
      { name: "task",    type: "string", description: "Tarea/actividad", required: true },
      { name: "subject", type: "string", description: "Asignatura",      required: false },
...
Incluye 4-5 criterios medibles.`
        ...
        return { success: true, output: `📊 **Rúbrica para "${task}"**:\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...

  {
    ...
    params: [
      ...
      { name: "lines", type: "number", description: "N° puntos",       required: false },
    ],
    async execute(args, baseUrl, options) {
      try {
        ...
        const res = await callInternalAPI(baseUrl, "/api/superagent/chat", {
          messages: [{ role: "user", content: `Resume el siguiente texto en ${lines} puntos clave, en español:\n\n${text}` }],
...

  {
    ...
    async execute(args, baseUrl, options) {
      try {
        ...
        const res = await callInternalAPI(baseUrl, "/api/superagent/chat", {
          messages: [{ role: "user", content: `Traduce al ${target}. Devuelve SOLO la traducción:\n\n${text}` }],
          task: "fast",
          maxTokens: 2000,
        }, options)
        const data = await res.json()
...

  {
    ...
    enabled:     true,
    params: [
      { name: "concept", type: "string", description: "Concepto o escena", required: true },
    ...
    async execute(args, baseUrl, options) {
      try {
        ...
        const res = await callInternalAPI(baseUrl, "/api/superagent/chat", {
          messages: [{ role: "user", content: `Crea un prompt en inglés para generar una imagen educativa.\nConcepto: "${concept}"\nEstilo: ${style}\nEl prompt debe ser detallado, apto para FLUX/Stable Diffusion, sin explicaciones.` }],
          task: "fast",
          maxTokens: 400,
        }, options)
        const data = await res.json()
...

  {
    ...
    icon:        "🎨",
    description: "Genera una imagen desde el chat usando el motor unificado de Image Studio.",
    category:    "media",
    enabled:     true,
    params: [
    ...
    async execute(args, baseUrl, options) {
      try {
        ...
        const res = await callInternalAPI(baseUrl, "/api/agents/imagenes", {
          prompt,
          style,
          width: 1024,
          height: 576,
          ...
          mode: "quality",
          source: "superagent",
          educationalContext: "Imagen solicitada desde EduAI Claw para uso educativo.",
        }, options)
...

  {
    ...
    enabled:     true,
    params: [
      { name: "text",    type: "string", description: "Texto para narrar", required: true },
      { name: "speaker", type: "string", description: "Voz A/B",          required: false,
        options: ["A", "B"] },
    ],
    async execute(args, baseUrl, options) {
      try {
        const text = String(args.text || args.content || "").trim().slice(0, 1200)
        const speaker = normalizeText(args.speaker) === "B" ? "B" : "A"
        const res = await callInternalAPI(baseUrl, "/api/agents/tts", {
          text,
          speaker,
          addMotivation: false,
        }, options)
...

  {
    ...
    enabled:     true,
    params: [
      { name: "prompt",   type: "string",  description: "Descripción del video", required: true },
      { name: "style",    type: "string",  description: "Estilo",                required: false },
      { name: "duration", type: "number",  description: "Duración",              required: false },
      { name: "withAudio",type: "boolean", description: "Incluir audio",          required: false },
    ],
    async execute(args, baseUrl, options) {
      try {
        const prompt = normalizeText(args.prompt || args.content)
        ...
        const res = await callInternalAPI(baseUrl, "/api/agents/video", {
          prompt,
          style,
          duration,
          withAudio,
...

  {
    ...
    label:       "Música focus",
    icon:        "🎵",
    description: "Recomienda música de concentración según actividad y asignatura.",
    category:    "media",
    enabled:     true,
    params: [
      { name: "activity", type: "string", description: "Actividad", required: true },
      { name: "subject",  type: "string", description: "Asignatura", required: false },
    ],
    async execute(args) {
      ...
      return {
        success: true,
        output: `🎵 **Sesión musical recomendada**\n\nActividad: ${activity || "estudio"}${subject ? `\nAsignatura: ${subject}` : ""}\n\n${recommendation}\n\nTambién puedes usar el módulo [Exam Focus](/exam-focus), que integra Pomodoro, música y EduAI Claw.`,
      }
    },
  },

  {
    ...
    icon:        "💻",
    description: "Genera componentes, rutas API, queries Supabase o utilidades TypeScript.",
    category:    "code",
    enabled:     true,
    params: [
      { name: "request", type: "string", description: "Qué código generar", required: true },
    ],
    async execute(args, baseUrl, options) {
      try {
        const request = normalizeText(args.request || args.content)
        const res = await callInternalAPI(baseUrl, "/api/superagent/chat", {
          messages: [{ role: "user", content: `Genera código production-ready para EduAI Platform (Next.js 16, TypeScript, Supabase). Solicitud:\n${request}\n\nIncluye ruta exacta sugerida y código completo.` }],
          task: "coding",
          maxTokens: 4000,
        }, options)
        const data = await res.json()
        if (!data.success) throw new Error(data.error)
        return { success: true, output: `💻 **Código generado:**\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...

  {
    ...
    icon:        "🔧",
    description: "Analiza y corrige errores de TypeScript, React, Next.js o Supabase.",
    category:    "code",
    enabled:     true,
    params: [
      { name: "error", type: "string", description: "Error o código con problema", required: true },
    ],
    async execute(args, baseUrl, options) {
      try {
        const errorText = String(args.error || args.content || "").trim()
        const res = await callInternalAPI(baseUrl, "/api/superagent/chat", {
          messages: [{ role: "user", content: `Corrige este error/código de EduAI Platform. Entrega diagnóstico, archivo probable y parche exacto:\n\n${errorText}` }],
          task: "coding",
          maxTokens: 4000,
        }, options)
        const data = await res.json()
        if (!data.success) throw new Error(data.error)
        return { success: true, output: `🔧 **Corrección sugerida:**\n\n${data.text}` }
      } catch (err) {
        return {
          success: false,
...
export function detectToolFromMessage(message: string): ToolName | null {
  ...
  if (/corrige|arregla|soluciona|fix|error|bug|typescript|build failed/i.test(m) && /c[oó]digo|tsx|ts|react|next|supabase|api|error/i.test(m)) return "fix_code_error"
  if (/genera(r)?\s*(c[oó]digo|componente|ruta|api|query|funci[oó]n)|crea(r)?\s*(componente|api|query|c[oó]digo)/i.test(m)) return "generate_code"
  if (/genera(r)?\s*(un\s*)?(video|animaci[oó]n)|video educativo|text(o)? a video|imagen a video/i.test(m)) return "generate_edu_video"
  if (/m[uú]sica|focus|concentraci[oó]n|pomodoro|estudiar con m[uú]sica/i.test(m)) return "recommend_focus_music"
  if (/narr(a|ar|e)|escuchar|lee en voz|audio/i.test(m)) return "narrate_text"
  if (/genera(r)?\s*(una\s*)?(imagen|ilustraci[oó]n|afiche|poster)|crear\s*(imagen|ilustraci[oó]n)/i.test(m)) return "generate_image"
  ...
  if (/pie|nee|dislexia|tdah|tea|baja\s*visi[oó]n|adapta/i.test(m) && /text|pregunta|contenid/i.test(m)) return "adapt_for_pie"
  if (/planifica(ci[oó]n|r|)\s*(de\s*)?(clase|unidad|semana)/i.test(m)) return "plan_curriculum"
  ...
  if (/explic(a|ar)\s*(el|la|qu[eé]\s*es|concepto)/i.test(m)) return "explain_concept"
