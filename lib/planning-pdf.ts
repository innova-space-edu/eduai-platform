...
function drawFooter(doc: jsPDF, meta: PlanningPdfMeta) {
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    ...
    doc.text(`EduAI Platform · ${footerTitle} · Página ${page} de ${totalPages}`, margin, pageHeight - 5.8)
...
export async function exportPlanningPdf(meta: PlanningPdfMeta, content: string) {
  ...
  drawRoundedBox(doc, margin, y, contentWidth, 32, COLORS.white, COLORS.line, 6)
  ...
  doc.text(safe(meta.subtitle || "Documento generado desde el planificador curricular"), margin + 10, y + 25)
  y += 40

  const cardGap = 4
  const cardW = (contentWidth - cardGap * 3) / 4
  drawMetaCard(doc, margin, y, cardW, "Curso", meta.curso || "-", COLORS.softBlue, COLORS.blue)
  drawMetaCard(doc, margin + cardW + cardGap, y, cardW, "Asignatura", meta.asignatura || "-", COLORS.softGreen, COLORS.emeraldDark)
  drawMetaCard(doc, margin + (cardW + cardGap) * 2, y, cardW, "Mes", meta.mes || "-", COLORS.softAmber, COLORS.amber)
  drawMetaCard(doc, margin + (cardW + cardGap) * 3, y, cardW, "Sesiones", `${meta.sesiones || 1} · ${meta.duracionMinutos || 45} min`, COLORS.softViolet, COLORS.violet)
  y += 25
  ...
  if (meta.contexto) {
    ...
    doc.setFont("helvetica", "bold")
    doc.setFontSize(9.6)
    setText(doc, COLORS.emeraldDark)
