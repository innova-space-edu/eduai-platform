...
function fallbackPlan(input: {
  ...
  const caption = [input.genre, input.mood, input.prompt].filter(Boolean).join(", ").slice(0, 1800)
  return {
    title,
    caption,
...
export async function listSongJobs() {
...
export async function createSongJob(body: any) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: "No autenticado" }, { status: 401 })
...
export async function deleteSongJob(id: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: "No autenticado" }, { status: 401 })
