...
function sanitizeMetadata(value: unknown): EduAIAnalyticsMetadata {
...
export function trackEduAIEvent(input: EduAITrackInput) {
  ...
  const body = JSON.stringify({
    ...
    errorCode: typeof input.errorCode === "string" ? input.errorCode.slice(0, 120) : null,
  ...
  window.fetch("/api/analytics/track", {
    ...
    // La analítica nunca debe bloquear ni alterar una función educativa.
  })
}
